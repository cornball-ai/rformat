...
data_spineplot <- function(off = NULL, breaks = NULL, xlevels = xlevels, ylevels = ylevels, xaxlabels = NULL, yaxlabels = NULL, weights = NULL) {
    fun <- function(settings, ...) {
        ...
        if (is.factor(datapoints$x)) {
            ...
            if (length(breaks) == 1L) {
                if (!is.numeric( { stop("invalid number of 'breaks'") }
                if (breaks > 1e6) {
                    warning(gettextf("'breaks = %g' is too large and set to 1e6",
                                     breaks))
                    breaks <- 1000000L
                }
                rg <- if (is.null(weights)) range(x,
                                                  na.rm = TRUE) else range(x[weights > 0],
                                                                           na.rm = TRUE)
        ...
        if (!is.null(xlevels) && x.categorical) {
            ...
            if (anyNA(xlevels) ||
                !all(xlevels %in% levels(datapoints$x))) warning("not all 'xlevels' correspond to levels of 'x'")
        ...
        if (!is.null(ylevels)) {
            ...
            if (anyNA(ylevels) ||
                !all(ylevels %in% levels(datapoints$y))) warning("not all 'ylevels' correspond to levels of 'y'")
        ...
        datapoints <- Map(function(dat, x.categorical, off) {
                ...
                x <- dat$x
                if (!x.categorical) {
                    x <- cut(as.numeric(x), breaks = breaks,
                             include.lowest = TRUE)
                }
                ...
                if (is.null(dat$weights)) {
                    ...
                    tab <- as.table(tapply(dat$weights, list(x, dat$y),
                                           FUN = sum, na.rm = TRUE))
                    tab[is.na(tab)] <- 0
                }
                nx <- nrow(tab)
                ...
                yat <- rbind(0,
                             apply(proportions(tab[, ny:1L, drop = FALSE], 1L),
                                   1L, cumsum))
                yat[is.na(yat)] <- 1
        ...
        ## axis labels
        yaxlabels <- if (is.null(yaxlabels)) levels(y) else rep_len(yaxlabels,
                                                                    ny)
        if (!is.null(yaxb)) {
        ...
        if (x.categorical) {
            ...
            xaxlabels <- if (is.null(xaxlabels)) {
                if (is.numeric(x)) breaks else c(x[1L],
                                                 x[c(diff(as.numeric(x)) > 0,
                                                     TRUE)])
            } else {
        ...
        # catch for x_by / y/by
        if (isTRUE(x_by)) datapoints$by <- factor(rep(xaxlabels,
                                                      each = ny)) # each x label extends over ny rows
        if (isTRUE(y_by)) datapoints$by <- factor(rep_len(yaxlabels,
                                                          nrow(datapoints)))
...
draw_spineplot <- function(tol.ylab = 0.05, off = NULL, col = NULL, xaxlabels = NULL, yaxlabels = NULL) {
    fun <- function(ixmin, iymin, ixmax, iymax, ilty, ilwd, icol, ibg,
        ...
        if (is.null(col)) {
            ...
            if (isFALSE(y_by)) {
                ibg <- if (isTRUE(grayscale)) gray.colors(ny) else seq_palette(ibg,
                                                                               ny)
            }
            ibg <- rep_len(ibg, ny)
        } else {
        ...
        if (type_info[["axes"]]) {
            ...
            yat <- if (equidist) seq.int(1 / (2 * ny), 1 - 1 / (2 * ny),
                                         by = 1 / ny) else (yat[- 1L] + yat[- length(yat)]) / 2
            ...
            if (is_facet_position(if (flip) "bottom" else "right", ifacet,
                                  facet_window_args)) spine_axis(if (flip) 1 else 4,
                                                                 type = type_info[["yaxt"]],
                                                                 categorical = FALSE)
        }
