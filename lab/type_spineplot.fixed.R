...
#'
#' @export
type_spineplot <- function (breaks = NULL, tol.ylab = 0.05, off = NULL,
                            xlevels = NULL, ylevels = NULL, col = NULL,
                            xaxlabels = NULL, yaxlabels = NULL,
                            weights = NULL) {
    col <- col
    out <- list(data = data_spineplot(off = off, breaks = breaks,
                                      xlevels = xlevels, ylevels = ylevels,
                                      xaxlabels = xaxlabels,
                                      yaxlabels = yaxlabels, weights = weights),
                draw = draw_spineplot(tol.ylab = tol.ylab, off = off,
                                      col = col, xaxlabels = xaxlabels,
                                      yaxlabels = yaxlabels),
                name = "spineplot")
...
data_spineplot <- function (off = NULL, breaks = NULL, xlevels = xlevels,
                            ylevels = ylevels, xaxlabels = NULL,
                            yaxlabels = NULL, weights = NULL) {
    fun <- function (settings, ...) {
        env2env(settings, environment(),
                c("datapoints", "xlim", "ylim", "facet", "facet.args", "by",
        ...
        if (is.factor(datapoints$x)) {
            ...
            if (is.null(xlim)) {
                xlim <- c(0, 1 + (nlevels(datapoints$x) - 1L) * off)
            }
        } else {
            off <- 0
            ...
            if (is.null(breaks)) {
                if (!is.null(xaxb)) {
                    ...
                    if (is.null(weights)) {
                        ...
                        breaks <- ceiling(log2(sum(weights)) + 1)
                    }
                }
            }
            ...
            if (length(breaks) == 1L) {
                ...
                if (is.null(weights)) {
                    ...
                    rg <- range(x[weights > 0], na.rm = TRUE)
                }
        ...
        if (!is.null(xlevels) && x.categorical) {
            if (is.numeric(xlevels)) {
                xlevels <- levels(datapoints$x)[xlevels]
            } else {
                xlevels <- xlevels
            }
            if (anyNA(xlevels) ||
                !all(xlevels %in% levels(datapoints$x))) {
        ...
        if (!is.null(ylevels)) {
            if (is.numeric(ylevels)) {
                ylevels <- levels(datapoints$y)[ylevels]
            } else {
                ylevels <- ylevels
            }
            if (anyNA(ylevels) ||
                !all(ylevels %in% levels(datapoints$y))) {
        ...
        if (isTRUE(x_by) || isTRUE(y_by)) {
            ...
            datapoints <- split(datapoints,
                                list(datapoints$by, datapoints$facet))
            datapoints <- Filter(function (k) nrow(k) > 0, datapoints)
        }
        ...
        datapoints <- Map(function (dat, x.categorical, off) {
            ...
            if (is.null(dat$weights)) {
                ...
                tab <- as.table(tapply(dat$weights, list(x, dat$y), FUN = sum,
                                       na.rm = TRUE))
                tab[is.na(tab)] <- 0
            }
            nx <- nrow(tab)
            ...
            yat <- rbind(0,
                         apply(proportions(tab[, ny:1L, drop = FALSE], 1L), 1L,
                               cumsum))
            yat[is.na(yat)] <- 1
        ...
        nx <- attr(datapoints[[1]],
                   "nx") ## should be the same for all by/facet groups
        ...
        if (is.null(yaxlabels)) {
            ...
            yaxlabels <- rep_len(yaxlabels, ny)
        }
        if (!is.null(yaxb)) {
        ...
        if (x.categorical) {
            ...
            xaxlabels <- if (is.null(xaxlabels)) {
                if (is.numeric(x)) {
                    ...
                    c(x[1L], x[c(diff(as.numeric(x)) > 0, TRUE)])
                }
            } else {
        ...
        # catch for x_by / y/by
        if (isTRUE(x_by)) {
            datapoints$by <- factor(rep(xaxlabels,
                                        each = ny)) # each x label extends over ny rows
        }
        if (isTRUE(y_by)) {
            datapoints$by <- factor(rep_len(yaxlabels, nrow(datapoints)))
        }

        ## grayscale flag
        grayscale <- null_by && null_palette &&
            is.null(.tpar[["palette.qualitative"]])
        ...
        yaxs <- "i"
        ylabs <- yaxlabels
        type_info <- list(off = off, x.categorical = x.categorical, nx = nx,
                          ny = ny, xat = xat, yat = yat, xaxlabels = xaxlabels,
                          yaxlabels = yaxlabels, breaks = breaks,
                          axes = axes_orig, xaxt = xaxt_orig, yaxt = yaxt_orig,
                          grayscale = grayscale, x_by = x_by, y_by = y_by)
        ...
        env2env(environment(), settings,
                c("x", "y", "ymin", "ymax", "xmin", "xmax", "col", "bg",
                  "datapoints", "by", "facet", "axes", "frame.plot", "xaxt",
                  "yaxt", "xaxs", "yaxs", "ylabs", "type_info", "facet.args"))
...
draw_spineplot <- function (tol.ylab = 0.05, off = NULL, col = NULL,
                            xaxlabels = NULL, yaxlabels = NULL) {
    fun <- function (ixmin, iymin, ixmax, iymax, ilty, ilwd, icol, ibg, flip,
                     facet_window_args, type_info, ifacet, ...) {
        ...
        if (type_info[["xaxt"]] %in% c("l", "t", "n") &&
            type_info[["yaxt"]] %in% c("l", "t", "n") &&
                !all(c(type_info[["xaxt"]],
                       type_info[["yaxt"]]) == "n")) { ilwd <- 0 }
        ...
        if (type_info[["axes"]]) {
            if (x.categorical) {
                spine_axis(if (flip) 2 else 1,
                           at = (xat[1L:nx] + xat[2L:(nx + 1L)] - off) / 2,
                           labels = xaxlabels, type = type_info[["xaxt"]],
                           categorical = TRUE)
            } else {
            ...
            if (equidist) {
                yat <- seq.int(1 / (2 * ny), 1 - 1 / (2 * ny), by = 1 / ny)
            } else {
                yat <- (yat[- 1L] + yat[- length(yat)])
            }
            spine_axis(if (flip) 3 else 2, at = yat, labels = yaxlabels,
                       type = type_info[["yaxt"]], categorical = TRUE)
            if (is_facet_position(if (flip) "bottom" else "right", ifacet,
                                  facet_window_args)) {
                spine_axis(if (flip) 1 else 4, type = type_info[["yaxt"]],
                           categorical = FALSE)
            }
        }
...
to_hcl <- function (x) {
    ...
    x <- cbind(H = atan2(x[, 3L], x[, 2L]) * 180 / pi,
               C = sqrt(x[, 2L] ^ 2 + x[, 3L] ^ 2), L = x[, 1L])
    x[is.na(x[, 1L]), 1L] <- 0
...
seq_palette <- function (x, n, power = 1.5) {
    ...
    alpha <- attr(x, "alpha")
    hcl(h = x[1L],
        c = seq.int(from = x[2L] ^ (1 / power), to = 0,
                    length.out = n + 1)[1L:n] ^ power,
        l = 100 - seq.int(from = (100 - x[3L]) ^ (1 / power),
                          to = pmin(8, (100 - x[3L]) / 2) ^ (1 / power),
                          length.out = n) ^ power,
        alpha = alpha)[1L:n]
}
