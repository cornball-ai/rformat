...
VARIMA <- function (formula, identification = NULL, ...) {
    ...
    varima_model <- new_model_class("VARIMA", train = train_varima,
        specials = specials_varima, origin = NULL, check = all_tsbl_checks)
...
residuals.VARIMA <- function (object, ...) {
    rbind(
        matrix(NA,
            nrow(object$data) - nrow(object$residuals) + nrow(object$y_end),
            ncol(object$residuals)),
        object$residuals)
}

#' @export
model_sum.VARIMA <- function (x) {
    sprintf("VARIMA(%i,%i,%i)%s", varima_order(x$Phi), NROW(x$y_end),
        varima_order(x$Theta), ifelse(x$cnst, " w/ mean", ""))
}
...
tidy.VARIMA <- function (x, ...) {
    ...
    col2row <- function (x, k = NROW(x)) {
        ...
        matrix(x[as.numeric(t(irow + icol + ilag))], nrow = k * i, ncol = k)
    }
    ...
    rownames(coef_mat) <- c(if (x$cnst) "constant" else NULL, paste0("lag(", rep(cn, each = p), ",", rep.int(seq_len(p), k), ")"), paste0("lag(e_", rep(cn, each = q), ",", rep.int(seq_len(q), k), ")"))

    rdf <- (p + q) * k ^ 2 + k * (k + 1) / 2

    coef <- dplyr::as_tibble(coef_mat, rownames = "term")
    coef <- tidyr::gather(coef, ".response", "estimate",
        !!!syms(colnames(coef_mat)))
...
glance.VARIMA <- function (x, ...) {
    tibble(sigma2 = list(x$Sigma), kronecker_indices = list(x$Kindex),
        AIC = x$aic, BIC = x$bic)
}
...
report.VARIMA <- function (object, ...) {
    ...
    coef <- map(coef,
        function (x) `colnames<-`(rbind("Est." = x$estimate #, s.e. = x$std.error), x$term)
    )
    ...
    cat(sprintf("\nAIC = %s\tBIC = %s", format(round(object$aic, 2L)), format(round(object$bic, 2L))))
}
...
IRF.VARIMA <- function (x, new_data, specials, impulse = NULL, orthogonal = FALSE, ...) {
    ...
    if (".impulse" %in% names(new_data)) {
        ...
        new_data$.innov <- matrix(0, nrow = nrow(new_data),
