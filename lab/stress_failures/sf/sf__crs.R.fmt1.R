...
Ops.crs <- function (e1, e2) {
    ...
    !(e1 == e2)
        ...
        if (is.na(e1) || is.na(e2)) { # only one of them is NA_crs_
            FALSE
        } else {
...
make_crs <- function (x) {
    if (inherits(x, "CRS")) {
        x <- if (is.null(comment(x)) ||
            (CPL_proj_version() < "6.0.0" || CPL_gdal_version() < "3.0.0"))
    ...
    x
        ...
        if (grepl("+init=epsg:", x) &&
            ...
            compareVersion(sf_extSoftVersion()[["proj.4"]],
                "6.3.1") < 0) { # nocov start FIXME:
            x <- strsplit(x, " ")[[1]]
            if (length(x) > 1)
            warning(paste("the following proj4string elements are ignored:", paste(x[-1], collapse = " "), "; remove the +init=epsg:XXXX to undo this"))
...
`st_crs<-.sfc` <- function (x, value) {
    ...
    if (is.na(end_crs) && !is.na(start_crs) &&
        isTRUE(st_is_longlat(start_crs)) && any(st_is_full(x)))
    ...
    NA
        ...
        if (ret && inherits(x, c("sf", "sfc", "stars")) &&
            ...
            if (all(!is.na(unclass(bb))) &&
                (bb["xmin"] < (-180 - eps) || bb["xmax"] > (360 + eps) ||
                    bb["ymin"] < (-90 - eps) || bb["ymax"] > (90 + eps)))
...
udunits_from_proj <- list(
    ...
    `ch` = as_units("chain"),
    `link` = as_units("link",
        check_is_valid = FALSE), # not (yet) existing; set in .onLoad()
...
st_as_text.crs <- function (x, ..., projjson = FALSE, pretty = FALSE) {
    ...
    NA_character_
        ...
        if (compareVersion(sf_extSoftVersion()["GDAL"], "3.1.0") == -1 ||
            compareVersion(sf_extSoftVersion()["proj.4"], "6.2.0") == -1)
...
#' @export
NA_crs_ <- structure(list(input = NA_character_, wkt = NA_character_),
    class = "crs")
...
`$.crs` <- function (x, name) {
    ...
    x[[name]]
        ...
        p[[name]]
            ...
            if (is.na(n)) {
                tryNum<-function(x){n <- x
            } else {
                tryNum<-function(x){n <- n }
            }
