...
st_sfc <- function (..., crs = NA_crs_, precision = 0.0, check_ring_dir = FALSE, dim,
    ...
    if (length(lst) == 1 && is.list(lst[[1]]) && !inherits(lst[[1]], "sfg") && (length(lst[[1]]) == 0 || inherits(lst[[1]][[1]], "sfg") || is.null(lst[[1]][[1]])))
    lst <- lst[[1]]
...
"[.sfc" <- function (x, i, j, ..., op = st_intersects) {
    ...
    if (!is.null(dim(x))) { # x is an array with geometries
        ...
        # x is a list but avoid NextMethod() to allow j, ... to be specified & ignored:
    }
...
st_geometry_type <- function (x, by_geometry = TRUE) {
    ...
    factor(f, levels = c("GEOMETRY", "POINT", "LINESTRING", "POLYGON", "MULTIPOINT", "MULTILINESTRING", "MULTIPOLYGON", "GEOMETRYCOLLECTION", "CIRCULARSTRING", "COMPOUNDCURVE", "CURVEPOLYGON", "MULTICURVE", "MULTISURFACE", "CURVE", "SURFACE", "POLYHEDRALSURFACE", "TIN", "TRIANGLE"))
}
...
typed_empty <- function (cls, ncol = 2, dim = "XY") {
    switch(cls[1], sfc_POINT = st_point(rep(NA_real_, ncol), dim = dim), sfc_MULTIPOINT = st_multipoint(matrix(numeric(0), ncol = ncol), dim = dim), sfc_LINESTRING = st_linestring(matrix(numeric(0), ncol = ncol), dim = dim), sfc_MULTILINESTRING = st_multilinestring(dim = dim), sfc_POLYGON = st_polygon(dim = dim), sfc_MULTIPOLYGON = st_multipolygon(dim = dim), st_geometrycollection(dims = dim))
}
...
st_coordinates.sfc <- function (x, ...) {
    ...
    ret <- switch(class(x)[1], sfc_POINT = matrix(unlist(x, use.names = FALSE), nrow = length(x), byrow = TRUE, dimnames = NULL), sfc_MULTIPOINT =, sfc_LINESTRING = coord_2(x), sfc_MULTILINESTRING =, sfc_POLYGON = coord_3(x), sfc_MULTIPOLYGON = coord_4(x), stop(paste("not implemented for objects of class", class(x)[1])))
    Dims <- class(x[[1]])[1]
    ncd <- nchar(Dims)
