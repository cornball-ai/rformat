...
#' }
#' @export
run <- function (file = "index.Rmd", dir = dirname(file),
                 default_file = NULL, auto_reload = TRUE, shiny_args = NULL,
                 render_args = NULL) {
    ...
    if (is_shiny_prerendered(runtime, server)) {
        ...
        app <- shiny::shinyApp(ui = rmarkdown_shiny_ui(dir, default_file),
            ...
            onStart = onStart,
            server = rmarkdown_shiny_server(dir, default_file, auto_reload,
                render_args))
    ...
    shiny_args <- merge_lists(
        list(appDir = app, launch.browser = launch_browser), shiny_args)
...
rmarkdown_shiny_server <- function (dir, file, auto_reload, render_args) {
    function (input, output, session) {
        ...
        doc <- shiny::reactive({
            ...
            args <- merge_lists(
                list(input = reactive_file(), output_file = output_dest,
            ...
            dependencies <- append(dependencies,
                list(create_performance_dependency(resource_folder)))
...
rmarkdown_shiny_ui <- function (dir, file) {
    function (req) {
        ...
        tags$div(
            tags$head(tags$script(src = "rmd_resources/rmd_loader.js"),
                tags$link(href = "rmd_resources/rmd_loader.css",
                    rel = "stylesheet")),
            ...
            shiny::conditionalPanel("!output.__reactivedoc__",
                tags$div(id = "rmd_loader_wrapper",
...
shinyHTML_with_deps <- function (html_file, deps) {
    ...
    if (nzchar(Sys.getenv("RSTUDIO"))) {
        ...
        if (nzchar(local_mathjax)) {
            html <- gsub(
                pattern = "https://mathjax.rstudio.com/latest/MathJax.js?",
                replacement = "mathjax-local/MathJax.js?", x = html,
...
rmd_cached_output <- function (input) {
    ...
    if (tolower(xfun::file_ext(input)) %in% c("htm", "html")) {
        return(list(cacheable = TRUE, cached = TRUE, dest = "", shiny_html = shinyHTML_with_deps(input, NULL), resource_folder = ""))
    }
    ...
    if (!is_shiny_classic(front_matter$runtime)) {
        ...
        output_dest <- paste(
            file.path(basetmp, "rmarkdown", output_key,
    ...
    list(cacheable = cacheable, cached = cached, dest = output_dest,
        shiny_html = shiny_html, resource_folder = resource_folder)
}
