...
as.numeric(sub(".*([-+]?\\d+(\\.\\d*)?([Ee][-+]?\\d+)?){1}.*", "\\1", familyString))
...
setNBdisp <- function (object, theta) {
    ...
    rr <- object@resp
    newresp <- do.call(glmResp$new, c(lapply(setNames(nm = glmNB.to.change), rr$field), list(family = MASS::negative.binomial(theta = theta))))
    ...
    control = NULL)
{
    ...
    NBfun <- function (t) {
        ...
        lastfit <<- f_refitNB(lastfit, theta = exp(t), control = control)
    ...
    eps = .Machine$double.eps ^ 0.25, trace = 0)
{
    ...
    theta.ml(Y, mu, weights = object@resp$weights, limit = limit, eps = eps, trace = trace)
}
    ...
    initCtrl = list(limit = 20, eps = 2 * tol, trace = verbose, theta = NULL))
{
    ...
    if (is.null(th <- initCtrl$theta)) {
        ...
        g0 <- suppressMessages(eval(mc, parent.frame(1L)))

        th <- est_theta(g0, limit = initCtrl$limit, eps = initCtrl$eps, trace = initCtrl$trace)
    ...
    ## ** see FIXME above
    g1 <- suppressMessages(eval(mc, parent.frame(1L)))
    ...
    res <- optTheta(g1, interval = interval, tol = tol, verbose = verbose, control = c(control, nb.control))
    res
}
