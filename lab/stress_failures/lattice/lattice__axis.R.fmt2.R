...
function (lim, packet.number = 0, packet.list = NULL, right = TRUE, ...) {
    ...
    calculateAxisComponents(lim, packet.list = packet.list,
        packet.number = packet.number, ...)
    list(num.limit = comps$num.limit,
        left = list(ticks = list(at = comps$at, tck = 1),
            labels = list(at = comps$at, labels = comps$labels, cex = 1,
                check.overlap = comps$check.overlap)),
        right = right)
}
...
axis.default <-
function (side = c("top", "bottom", "left", "right"), scales, components,
          as.table, labels = c("default", "yes", "no"),
    ...
    determineStatus <- function (x) {
        ...
        ## draw something here
        if (is.null(x) || (is.logical(x) && !x)) { FALSE } else { TRUE }
    }
    lastPanel <- function () {
    ...
    atBoundary <- function () {
        switch(side, top = if (as.table) row == 1 else row == layout.dim[1], bottom = if (!as.table) row == 1 else row == layout.dim[1], left = column == 1, right = column == layout.dim[2] || lastPanel())
    }

    ## FIXME: what about scales$relation ?
    do.ticks <-
    switch(ticks, yes = TRUE, no = FALSE, default = scales$draw &&
        determineStatus(components[[side]]) &&
    ...
    switch(labels,
        ...
        switch(side,
            top = rep(scales$alternating,
                length.out = column)[column] %in% c(2, 3),
            bottom = rep(scales$alternating,
                length.out = column)[column] %in% c(1, 3),
    ...
    {
        comp.list <-
        switch(side, top = if (is.logical(components[["top"]]) && components[["top"]]) components[["bottom"]] else components[["top"]], bottom = components[["bottom"]], left = components[["left"]], right = if (is.logical(components[["right"]]) && components[["right"]]) components[["left"]] else components[["right"]])
        scales.tck <-
        switch(side, left =, bottom = scales$tck[1], right =,
            top = scales$tck[2])
        if (!is.logical(comp.list)) ## must be FALSE if it is
        {
            if (do.ticks)
            panel.axis(side = side, at = comp.list$ticks$at, labels = FALSE,
                draw.labels = FALSE, check.overlap = FALSE, outside = TRUE,
                ticks = TRUE, tck = scales.tck * comp.list$ticks$tck, ...)
            if (do.labels)
            panel.axis(side = side, at = comp.list$labels$at,
                labels = comp.list$labels$labels, draw.labels = TRUE,
                check.overlap = comp.list$labels$check.overlap, outside = TRUE,
                ticks = FALSE, tck = scales.tck * comp.list$ticks$tck, ...)
        }
    }
}
...
function (x, ..., packet.number, packet.list, abbreviate = NULL, minlength = 4) ## This aims to be a general function which given a general
...
{
    if (all(is.na(x)))
    return(list(at = numeric(0), labels = numeric(0), check.overlap = TRUE, num.limit = c(0, 1)))
...
formattedTicksAndLabels <- function (x, ...) UseMethod("formattedTicksAndLabels")
...
function (x, at = FALSE, used.at = NULL, labels = FALSE, logsc = FALSE, ...,
          ...
          format.posixt = NULL, equispaced.log = TRUE) ## meant for when x is numeric
{
    rng <-
    if (length(x) == 2) { as.numeric(x) } else { range(as.numeric(x)) }
    ...
    if (have.log)
    logbase <-
    if (is.numeric(logsc)) { logsc } else
    if (logsc == "e") { exp(1) } else { stop("Invalid value of 'log'") }
    logpaste <-
    if (have.log) { paste(as.character(logsc), "^", sep = "") } else { "" }

    ## will check for overlap only when neither at nor labels is specified

    check.overlap <-
    if (is.logical(at) && is.logical(labels)) { TRUE } else { FALSE }

    if (is.logical(at)) ## at not explicitly specified
    {
        at <-
        if (have.log && !equispaced.log) { # FIXME: num.limit instead of rng?
            checkArgsAndCall(axisTicks,
                list(usr = log10(logbase ^ rng), log = TRUE, axp = NULL, ...))
        } else {
...
function (x, at = FALSE, used.at = NULL, ..., num.limit = NULL,
          labels = FALSE, logsc = FALSE, abbreviate = NULL, minlength = 4,
          format.posixt = NULL) {
    ...
    logbase <-
    if (is.numeric(logsc)) { logsc } else { exp(1) }
    ...
    list(at = at, labels = if (is.logical(labels)) datelabels else labels, check.overlap = check.overlap, num.limit = if (length(x) == 2) as.numeric(x) else as.numeric(range(x)))
}
...
function (x, at = FALSE, used.at = NULL, labels = FALSE, logsc = FALSE, ...,
    ...
    if (!is.logical(labels)) ## no need to do anything
    {
        ...
        return(list(at = at, labels = labels, check.overlap = FALSE, num.limit = num.lim))
    }
    ...
    list(at = as.numeric(at), labels = labels, check.overlap = mat,
        num.limit = num.lim)
}
    ...
    if (is.logical(labels)) { ## labels not specified
        if (is.null(format.posixt)) { ## format not specified
            ...
            if (max(abs(at)) <= 1) { ## 'x' might exceed 1
                ...
                labels <- format(at, simplify = simplify)
            }
            ans$labels <- labels
        }
    }
...
function (x, at = FALSE, used.at = NULL, num.limit = NULL, labels = FALSE,
          logsc = FALSE, abbreviate = NULL, minlength = 4, simplify = TRUE,
          ..., format.posixt = NULL) {
    ...
    list(at = as.numeric(at), labels = labels, check.overlap = check.overlap, num.limit = if (length(x) == 2) as.numeric(x) else rng)
}

panel.axis <-
function (side = c("bottom", "left", "top", "right"),
          at = pretty(scale.range), labels = TRUE, draw.labels = TRUE,
          check.overlap = FALSE, outside = FALSE, ticks = TRUE,
          half = !outside,
          which.half = switch(side, bottom = "lower", left = "upper",
              top = "upper", right = "lower"),
          ...
          text.lineheight = axis.text$lineheight, line.col = axis.line$col,
    ...
    scale.range <-
    range(switch(side, left = cpl$ylim, top = cpl$xlim, right = cpl$ylim, bottom = cpl$xlim))
    ...
    if (is.logical(labels))
    labels <-
    if (labels) { format(at, trim = TRUE) } else { NULL } else if (at.missing)
    ...
    switch(side,
        bottom = grid.segments(x0 = unit(at[axid], "native"),
            x1 = unit(at[axid], "native"), y0 = unit(0, "npc"),
            y1 = orient.factor * tck.unit,
            name = trellis.grobname("ticks.bottom", type = "panel"),
            gp = gp.line),
        top = grid.segments(x0 = unit(at[axid], "native"),
            x1 = unit(at[axid], "native"), y0 = unit(1, "npc"),
            y1 = unit(1, "npc") - orient.factor * tck.unit,
            name = trellis.grobname("ticks.top", type = "panel"), gp = gp.line),
        left = grid.segments(y0 = unit(at[axid], "native"),
            y1 = unit(at[axid], "native"), x0 = unit(0, "npc"),
            x1 = orient.factor * tck.unit,
            name = trellis.grobname("ticks.left", type = "panel"),
            gp = gp.line),
        right = grid.segments(y0 = unit(at[axid], "native"),
            y1 = unit(at[axid], "native"), x0 = unit(1, "npc"),
    ...
    {
        {
            just <-
            if (outside)
            switch(side, bottom = if (rot[1] == 0) c("centre", "top") else c("right", "centre"), top = if (rot[1] == 0) c("centre", "bottom") else c("left", "centre"), left = if (rot[2] == 90) c("centre", "bottom") else c("right", "centre"), right = if (rot[2] == 90) c("centre", "top") else c("left", "centre"))
            else
            switch(side, bottom = if (rot[1] == 0) c("centre", "bottom") else c("left", "centre"), top = if (rot[1] == 0) c("centre", "top") else c("right", "centre"), left = if (rot[2] == 90) c("centre", "top") else c("left", "centre"), right = if (rot[2] == 90) c("centre", "bottom") else c("right", "centre"))
        }
        switch(side,
            bottom = grid.text(label = labels[axid],
                x = unit(at[axid], "native"), y = orient.factor * lab.unit,
                rot = rot[1], check.overlap = check.overlap, just = just,
                name = trellis.grobname("ticklabels.bottom", type = "panel"),
                gp = gp.text),
            top = grid.text(label = labels[axid], x = unit(at[axid], "native"),
                y = unit(1, "npc") - orient.factor * lab.unit, rot = rot[1],
                check.overlap = check.overlap, just = just,
                name = trellis.grobname("ticklabels.top", type = "panel"),
                gp = gp.text),
            left = grid.text(label = labels[axid],
                y = unit(at[axid], "native"), x = orient.factor * lab.unit,
                rot = rot[2], check.overlap = check.overlap, just = just,
                name = trellis.grobname("ticklabels.left", type = "panel"),
                gp = gp.text),
            right = grid.text(label = labels[axid],
                y = unit(at[axid], "native"),
                x = unit(1, "npc") - orient.factor * lab.unit, rot = rot[2],
                check.overlap = check.overlap, just = just,
                name = trellis.grobname("ticklabels.right", type = "panel"),
                gp = gp.text))
    }
    invisible()
}
