    ...
    right = TRUE, ...)
{
    ...
    hist(as.numeric(x), breaks = breaks, plot = FALSE, include.lowest = include.lowest, right = right)
    else
...
function (x,
    ...
    ...)
{
    if (length(x) < 1) { prepanel.null() } else {
        if (is.null(breaks))
        {
            breaks <-
            if (is.factor(x)) { seq_len(1 + nlevels(x)) - 0.5 } else
            if (equal.widths) {
        ...
        y <-
        switch(type, count = h$counts, percent = 100 * h$counts / length(x), density = h$density)
        list(xlim =
    ...
    identifier = "histogram")
{
    ...
    panel.lines(x = xscale[1] + diff(xscale) * c(0.05, 0.95), y = c(0, 0), col = border, lty = lty, lwd = lwd, alpha = alpha, identifier = paste(identifier, "baseline", sep = "."))
    ...
    if (length(x) > 0)
    {
        ...
        if (nb > 1)
        {
            panel.rect(x = breaks[-nb], y = 0, height = y, width = diff(breaks), col = col, alpha = alpha, border = border, lty = lty, lwd = lwd, just = c("left", "bottom"), identifier = identifier)
        }
    }
}
    ...
    subset = TRUE)
{
    ...
    form <-
    latticeParseFormula(formula, data, subset = subset, groups = groups, multiple = allow.multiple, outer = outer, subscripts = TRUE, drop = drop.unused.levels)
    ...
    foo <-
    do.call("trellis.skeleton", c(list(formula = formula, cond = cond, aspect = aspect, strip = strip, panel = panel, xlab = xlab, ylab = ylab, xlab.default = form$right.name, ylab.default = "dummy", lattice.options = lattice.options), dots), quote = TRUE)
    ...
    if (have.xlog)
    {
        ...
        xbase <-
        if (is.logical(xlog)) { 10 } else
        if (is.numeric(xlog)) { xlog } else
        if (xlog == "e") { exp(1) }
    ...
    foo$ylab.default <-
    switch(type, count = gettext("Count"), percent = gettext("Percent of Total"), density = gettext("Density"))
    ...
    c(list(breaks = breaks, type = type, equal.widths = equal.widths, nint = nint), dots)
    ...
    more.comp <- c(limits.and.aspect(default.prepanel, prepanel = prepanel, have.xlim = have.xlim, xlim = xlim, have.ylim = have.ylim, ylim = ylim, x.relation = foo$x.scales$relation, y.relation = foo$y.scales$relation, panel.args.common = foo$panel.args.common, panel.args = foo$panel.args, aspect = aspect, npackets = npackets, x.axs = foo$x.scales$axs, y.axs = foo$y.scales$axs), cond.orders(foo))
    foo[names(more.comp)] <- more.comp

    if (is.null(foo$legend) && needAutoKey(auto.key, groups))
    {
        foo$legend <-
        autoKeyLegend(list(text = levels(as.factor(groups)), points = FALSE, rectangles = TRUE, lines = FALSE), auto.key)
    }
    class(foo) <- "trellis"
    foo
}
