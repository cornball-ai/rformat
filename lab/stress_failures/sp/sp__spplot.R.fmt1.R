...
setMethod("sppanel", "SpatialPoints",
    function (obj, pch = 3, ...) panel.points(coordinates(obj), pch = pch, ...))
# backward compatibility:
...
sppanelList <- function (obj, p.number, first, ...) {
    ...
    if (is.character(obj[[1]]) || is.function(obj[[1]])) {
        ...
        lapply(obj[sp], function (x) {
            if (missingFirst || identical(obj$first, first) ||
                opaque(x) == first)
...
getFormulaLevelplot <- function (sdf, zcol) {
    if (length(zcol) > 1)
    as.formula(paste("z~", paste(dimnames(coordinates(sdf))[[2]], collapse = "+"), "|name"))
    else {
        if (!is.character(zcol))
        zcol <- names(sdf)[zcol]
        as.formula(paste(zcol, "~", paste(dimnames(coordinates(sdf))[[2]], collapse = "+")))
    }
}

spplot.grid <- function (obj, zcol = names(obj), ..., names.attr,
                         scales = list(draw = FALSE), xlab = NULL,
                         ylab = NULL, aspect = mapasp(obj, xlim, ylim),
                         panel = panel.gridplot, sp.layout = NULL, formula,
                         xlim = bbox(obj)[1, ylim = bbox(obj)[2,
                         checkEmptyRC = TRUE, col.regions = get_col_regions()) {
    ...
    args <- append(
        list(formula, data = as(sdf, "data.frame"), aspect = aspect,
...
spplot.polygons <- function (obj, zcol = names(obj), ..., names.attr,
    ...
    args <- append(
        list(formula, data = as(sdf, "data.frame"), aspect = aspect,
            grid.polygons = grid.polygons, panel = panel, xlab = xlab,
            ylab = ylab, scales = scales, sp.layout = sp.layout, xlim = xlim,
            ylim = ylim, col.regions = col.regions),
        list(...))
    if (all(unlist(lapply(obj@data[zcol], is.factor)))) {
        #if (!is.null(args$col.regions) &&
        #		nlevels(obj@data[[zcol[1]]]) != length(args$col.regions))
    ...
    col.regions = get_col_regions())
{
    ...
    if (missing(formula)) {
        if (length(zcol) > 1) {
            formula <- as.formula(paste(paste(dimnames(coordinates(sdf))[[2]][2:1], collapse = "~"), "|name"))
            sdf <- spmap.to.lev(sdf, zcol = zcol, names.attr = names.attr)
        } else {
            if (!is.character(zcol))
            zcol <- names(sdf)[zcol]
    ...
    scales <- longlat.scales(obj, scales, xlim, ylim)
    args.xyplot <- append(
        list(formula, data = as(sdf, "data.frame"), panel = panel,
            aspect = aspect, scales = scales, xlab = xlab, ylab = ylab,
            sp.layout = sp.layout, xlim = xlim, ylim = ylim,
            edge.col = edge.col, col.regions = col.regions),
        dots)
                ...
                if (get_Polypath()) {
                    ...
                    for (j in pOi) {
                        ...
                        gp <- gpar(fill = fill, alpha = alpha, col = col,
                            lwd = lwd, lty = lty)
                        grid.polygon(coords[, 1], coords[, 2],
...
mapLegendGrob <- function (obj, widths = unit(1, "cm"), heights = unit(1, "cm"),
    ...
    key.gf <- placeGrob(key.gf,
        rectGrob(gp = gpar(fill = "transparent", col = NULL)), row = NULL,
        col = NULL)
...
layout.north.arrow <- function (type = 1) {
    if (type == 1) {
        ...
        x2 <- c(0, 0.0967, 0.0967, 0.2928, 0.3908, 0.3908, 0.2928, 0.2928,
            0.1032, 0, 0)
        y1 <- c(0, 0, 0.8823, 0.8235, 1, 0.8235, 0.8823, 0)
        y2 <- c(0.2352, 0.2352, 0.5686, 0.2352, 0.2352, 0.7189, 0.7189, 0.3986,
            0.7189, 0.7189, 0.2352)
...
layout.scale.bar <- function (height = 0.05) {
    ...
    x2 <- x1 + 0.5
    y2 <- y1
    SpatialPolygons(list(Polygons(list(Polygon(cbind(x1, y1))), ID = "left"), Polygons(list(Polygon(cbind(rev(x2), rev(y2)))), ID = "right")))
}
# scale.bar = .scale.bar()
...
colorkey.factor <- function (f, colorkey = list(), doColorkey = TRUE) {
    ...
    if (doColorkey) {
        colorkey <- append(colorkey,
            list(labels = list(at = at.labels, labels = lf),
...
addNAemptyRowsCols <- function (obj) {
    ...
    missingpatt <- matrix(missingpatt, obj@grid@cells.dim[1],
        obj@grid@cells.dim[2], byrow = FALSE)
    ...
    if (n > 0) {
        ...
        obj <- SpatialPointsDataFrame(xy, newatt, obj@coords.nrs,
            obj@proj4string, FALSE)
    } else
    ...
    cex.key, edge.col, colorkey)
{
    ...
    if (is.numeric(z)) {
        ...
        if (length(cuts) == 1) {
            if (do.log) {
                lz <- log(z)
                cuts <- c(min(z[valid]),
                    exp(seq(min(lz[valid]), max(lz[valid]), length = cuts + 1))[2:(cuts)],
                    max(z[valid]))
            } else cuts <- seq(min(z[valid]), max(z[valid]),
                length.out = cuts + 1)
        }
    ...
    lst$legend <- colorkey
        ...
        lst$legend <- list(right = list(fun = draw.colorkey, args = list(key = list(col = col.regions, at = cuts), draw = FALSE)))
            ...
            lst$key <- dots$key
                ...
                if (edge.col != "transparent") {
                    lst$key <- append(lst$key,
                        list(
                            points = list(pch = rep(pch, length.out = n),
...
panel.RgoogleMaps <- function (map) {
    ...
    grid.raster(map$myTile, mean(bb[1,]), mean(bb[2,]), diff(bb[1,]),
        diff(bb[2,]), default.units = "native", interpolate = FALSE)
}
