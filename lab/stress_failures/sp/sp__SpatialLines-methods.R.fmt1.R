...
SpatialLines <- function (LinesList, proj4string = CRS(as.character(NA))) {
    ...
    res <- new("SpatialLines", bbox = .bboxSls(LinesList),
        proj4string = proj4string, lines = LinesList)
    res
}
...
plotSpatialLines <- function (SL, xlim = NULL, ylim = NULL, col = 1, lwd = 1,
                              lty = 1, add = FALSE, axes = FALSE, lend = 0,
                              ljoin = 0, lmitre = 10, ...,
                              setParUsrBB = FALSE) {
    if (!add)
    ...
    if (length(ljoin) != length(lst)) {
        ljoin <- rep(ljoin, length = length(lst))
    }
    if (length(lmitre) != length(lst)) {
        lmitre <- rep(lmitre, length = length(lst))
    }
...
lines.Lines <- function (x, y = NULL, ...) invisible(lapply(x@Lines, function (x, ...) lines(x, ...), ...))
...
SpatialLines2SpatialPointsDataFrame <- function (from) {
    ...
    L <- unlist(L)
    SpatialPointsDataFrame(spp,
        data.frame(Lines.NR = L3, Lines.ID = factor(L2), Line.NR = L))
}
...
print.SpatialLines <- function (x, ..., digits = getOption("digits"),
    ...
    show(x)
    pst <- paste(
        strwrap(paste("Coordinate Reference System (CRS) arguments:", proj4string(x))),
        collapse = "\n")
    cat(pst, "\n")
}
...
labels.SpatialLines <- function (object, labelCRS, side = 1:2, ...) {
    ...
    if (missing(labelCRS) &&
        !is.na(slot(slot(object, "proj4string"), "projargs")))
