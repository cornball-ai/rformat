GridTopology <- function (cellcentre.offset, cellsize, cells.dim) {
    new("GridTopology", cellcentre.offset = cellcentre.offset, cellsize = cellsize, cells.dim = as.integer(cells.dim))
}

# setMethod("show", "GridTopology", function(object) summary(object))

as.data.frame.GridTopology <- function (x, row.names, optional, ...) data.frame(cellcentre.offset = x@cellcentre.offset, cellsize = x@cellsize, cells.dim = x@cells.dim)
...
setReplaceMethod("coordnames", signature(x = "GridTopology", value = "character"), coordnamesGT)
...
points2grid <- function (points, tolerance = sqrt(.Machine$double.eps),
    ...
    n <- dimensions(points)
    ret <- new("GridTopology", cellcentre.offset = numeric(n), cellsize = numeric(n), cells.dim = as.integer(rep(1, n)))
    ...
    for (i in 1:n) { # loop over x, y, and possibly z
        ...
        difx <- diff(sux)
        if (length(difx) == 0) {
            warning(paste("cell size from constant coordinate", i, "possibly taken from other coordinate"))
            ret@cellsize[i] <- NA
