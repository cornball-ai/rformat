...
makelonlat <- function (x) {
    ...
    if (is.na(geo)) {
        ...
        if (geo) { return(x) }
        error("plet",
            "coordinate reference system is unknown and does not look like lon/lat")
    } else if (!geo) {
    ...
    if ((!is.null(tiles)) && (length(tiles) > 0)) {
        ...
        if (length(tiles) > 0) {
            for (i in 1:length(tiles)) {
                map <- leaflet::addProviderTiles(map, tiles[i],
                    group = tiles[i],
                    options = leaflet::tileOptions(noWrap = !wrap))
            }
        }
...
.get_leg <- function (v, type = "", dig.lab = 3, cols, breaks = NULL,
...
setMethod("plet", signature(x = "SpatVector"),
    function (x, y = "", col, main = y, cex = 1, lwd = 2, lty = NULL,
              border = "black", alpha = c(0.3, 1), popup = TRUE,
              label = FALSE, split = FALSE,
              tiles = c("Streets", "Esri.WorldImagery", "OpenTopoMap"),
              wrap = TRUE, legend = "bottomright", collapse = FALSE,
              type = NULL, breaks = NULL, breakby = "eqint", sort = TRUE,
              reverse = FALSE, map = NULL, fill = NULL, ...) {
    if (!is.null(fill)) {
    ...
    if (y == "") { # no legend
        ...
        if (g == "polygons") {
            map <- leaflet::addPolygons(map, data = x, label = lab,
                group = group, fillColor = cols, fillOpacity = alpha[1],
                opacity = alpha[2], popup = pop, color = border, weight = lwd,
                dashArray = lty, ...)
        } else if (g == "lines") {
            map <- leaflet::addPolylines(map, data = x, label = lab,
                group = group, color = cols, opacity = alpha[1], popup = pop,
                weight = lwd, dashArray = lty, ...)
        } else {
            map <- leaflet::addCircleMarkers(map, data = x, radius = cex,
                popup = pop, group = group, label = lab,
                fillOpacity = alpha[1], opacity = alpha[2], color = cols,
                weight = lwd, dashArray = lty, ...)
        }
        ...
        if (split) {
            ...
            for (i in seq_along(u)) {
                ...
                if (g == "polygons") {
                    map <- leaflet::addPolygons(map, data = s, label = lab[i],
                        group = u[i], fillColor = cols[i],
                        fillOpacity = alpha[1], opacity = alpha[2],
                        popup = pop, col = border, weight = lwd,
                        dashArray = lty, ...)
                } else if (g == "lines") {
                    map <- leaflet::addPolylines(map, data = s, label = lab[i],
                        group = u[i], col = cols[i], opacity = alpha[1],
                        popup = pop, weight = lwd, dashArray = lty, ...)
                } else {
                    map <- leaflet::addCircleMarkers(map, data = s,
                        label = lab[i], group = u[i], col = cols[i],
                        fillOpacity = alpha[1], opacity = alpha[2],
                        popup = pop, radius = cex, weight = lwd,
                        dashArray = lty, ...)
                }
            }
            if (length(tiles) > 1) {
            ...
            leg <- .get_leg(v, type = type, dig.lab = 3, cols = col,
                breaks = breaks, breakby = breakby, sort = sort,
                reverse = reverse, border = border, ...)
            pop <- lab <- NULL
            if (isTRUE(popup[1])) pop <- popUp(x)
            if (isTRUE(label[1])) lab <- v
            if (g == "polygons") {
                map <- leaflet::addPolygons(map, data = x, label = lab,
                    group = y, fillColor = leg$main_cols, opacity = alpha[2],
                    fillOpacity = alpha[1], col = leg$main_cols, popup = pop,
                    weight = lwd, dashArray = lty, ...)
            } else if (g == "lines") {
                map <- leaflet::addPolylines(map, data = x, label = lab,
                    group = y, col = leg$main_cols, popup = pop,
                    opacity = alpha[1], weight = lwd, dashArray = lty, ...)
            } else {
                map <- leaflet::addCircleMarkers(map, data = x, label = lab,
                    group = y, weight = lwd, dashArray = lty,
                    col = leg$main_cols, radius = cex, popup = pop,
                    fillOpacity = alpha[1], opacity = alpha[2], ...)
            }
            if (length(tiles) > 1) {
                map <- leaflet::addLayersControl(map, baseGroups = tiles,
        ...
        if ((!is.null(legend)) && (!is.null(leg))) {
            if (leg$legend_type != "") {
                ...
                map <- leaflet::addLegend(map, position = legend,
                    colors = leg$leg$fill,
                    labels = as.character(leg$leg$legend), opacity = op,
                    title = main)
            }
        }
        map
...
setMethod("plet", signature(x = "SpatVectorCollection"),
    ...
    for (i in 1:n) {
        ...
        if (y[i] == "") { # no legend
            ...
            if (g == "polygons") {
                map <- leaflet::addPolygons(map, data = v, weight = lwd[i],
                    dashArray = lty[i], fillColor = icols,
                    fillOpacity = alpha[1], col = border[i],
                    opacity = alpha[2], popup = pop, label = lab,
                    group = nms[i], ...)
            } else if (g == "lines") {
                map <- leaflet::addPolylines(map, data = v, weight = lwd[i],
                    dashArray = lty[i], opacity = alpha[1], col = icols,
                    group = nms[i], popup = pop, label = lab, ...)
            } else {
                map <- leaflet::addCircleMarkers(map, data = v,
                    radius = cex[i], popup = pop, label = lab,
                    fillOpacity = alpha[1], opacity = alpha[2], col = icols,
                    group = nms[i], weight = lwd[i], dashArray = lty[i], ...)
            }
        } else {
            field <- y[i]
            if (is.numeric(field)) {
            ...
            if (split) {
                ...
                for (j in seq_along(u)) {
                    ...
                    if (g == "polygons") {
                        map <- leaflet::addPolygons(map, data = s,
                            weight = lwd[i], dashArray = lty[i],
                            label = lab[j], group = u[j],
                            fillColor = leg$main_cols, opacity = alpha[2],
                            fillOpacity = alpha[1], col = border[i],
                            popup = pop, ...)
                    } else if (g == "lines") {
                        map <- leaflet::addPolylines(map, data = s,
                            weight = lwd[i], dashArray = lty[i],
                            label = lab[j], group = u[j], col = leg$main_cols,
                            popup = pop, opacity = alpha[1], ...)
                    } else {
                        map <- leaflet::addCircleMarkers(map, data = s,
                            label = lab[j], group = u[j], col = leg$main_cols,
                            radius = cex[i], popup = pop,
                            fillOpacity = alpha[1], opacity = alpha[2],
                            weight = lwd[i], dashArray = lty[i], ...)
                    }
                }
            } else { # do not split
                leg <- .get_leg(vals, type = type, dig.lab = 3,
                    cols = cols[[i]], breaks = breaks[i], breakby = breakby[i],
                    sort = sort[i], reverse = reverse[i], ...)
                pop <- lab <- NULL
                if (isTRUE(popup[i])) pop <- popUp(x[i])
                if (isTRUE(label[i])) lab <- vals
                if (g == "polygons") {
                    map <- leaflet::addPolygons(map, data = v, weight = lwd[i],
                        dashArray = lty[i], label = lab, group = nms[i],
                        fillColor = leg$main_cols, opacity = alpha[2],
                        fillOpacity = alpha[1], col = border[i], popup = pop,
                        ...)
                } else if (g == "lines") {
                    map <- leaflet::addPolylines(map, data = v,
                        weight = lwd[i], dashArray = lty[i], label = lab,
                        group = nms[i], col = leg$main_cols, popup = pop,
                        opacity = alpha[1], ...)
                } else {
                    map <- leaflet::addCircleMarkers(map, data = v,
                        label = lab, group = nms[i], col = leg$main_cols,
                        radius = cex[i], popup = pop, fillOpacity = alpha[1],
                        opacity = alpha[2], weight = lwd[i],
                        dashArray = lty[i], ...)
                }
                if ((!is.null(legend[i])) && (!is.null(leg))) {
                    if (leg$legend_type != "") {
                        ...
                        map <- leaflet::addLegend(map, position = legend[i],
                            colors = leg$leg$fill,
                            labels = as.character(leg$leg$legend),
                            opacity = op, title = main[i])
                    }
                }
            }
        }
...
setMethod("polys", signature(x = "leaflet"),
    function (x, y, col, lwd = 2, lty = NULL, border = "black",
              alpha = c(.3, 1), popup = TRUE, label = FALSE, fill = NULL, ...) {
    if (!is.null(fill)) {
        warn("plet", "argument 'fill' will be removed. Use 'alpha' instead")
        alpha[1] <- fill
    }
    ...
    if (inherits(y, "SpatVector")) {
        ...
        leaflet::addPolygons(x, data = y, weight = lwd, dashArray = lty,
            fillColor = col, fillOpacity = alpha[1], col = border,
            opacity = alpha[2], popup = popup, label = label, ...)
        ...
        for (i in 1:length(nms)) {
            x <- leaflet::addPolygons(x, data = y[i], weight = lwd[i],
                dashArray = lty[i], fillColor = cols[i],
                fillOpacity = alpha[1], col = border[i], opacity = alpha[2],
                popup = popup[i], label = label[i], group = nms[i], ...)
        }
        collapse <- FALSE
        leaflet::addLayersControl(x, overlayGroups = nms,
            options = leaflet::layersControlOptions(collapsed = collapse))
    } else {
...
setMethod("lines", signature(x = "leaflet"),
    ...
    if (inherits(y, "SpatVector")) {
        ...
        if (!(geomtype(y) %in% c("lines", "polygons"))) {
            error("lines",
                "SpatVector y must have either lines or polygons geometry")
        }
        ...
        lty <- rep_len(lty, n)
        for (i in 1:length(nms)) {
            x <- leaflet::addPolylines(x, data = y[i], weight = lwd[i],
                dashArray = lty[i], opacity = alpha, col = cols[i],
                group = nms[i], ...)
        }
        collapse <- FALSE
        leaflet::addLayersControl(x, overlayGroups = nms,
...
setMethod("points", signature(x = "leaflet"),
    function (x, y, col, border = col, cex = 1, lwd = 2, lty = NULL,
    ...
    leaflet::addCircleMarkers(x, data = y, radius = cex, popup = popup,
        label = label, opacity = alpha[2], fillColor = col,
        fillOpacity = alpha[1], col = borders, weight = lwd, dashArray = lty,
        ...)
...
make.panel <- function (x, maxcell) {
    ...
    crs(x,
        warn = FALSE) <- "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext +no_defs"
    ext(x) <- c(0, 1, 0, asp)
    #if (!is.null(add)) {
    #	e <- as.vector(ext(x))
...
.get_cls <- function (x, type = "", dig.lab = 3, cols, breaks = NULL,
                      breakby = "eqint", sort = TRUE, reverse = FALSE, ...) {
    if (is.null(type) || (type == "")) {
...
setMethod("plet", signature(x = "SpatRaster"),
    function (x, y = 1, col, alpha = 0.8, main = names(x),
              tiles = c("Streets", "Esri.WorldImagery", "OpenTopoMap"),
              wrap = TRUE, maxcell = 500000, stretch = NULL,
              legend = "bottomright", shared = FALSE, panel = FALSE,
              collapse = TRUE, type = NULL, breaks = NULL, breakby = "eqint",
              range = NULL, fill_range = FALSE, map = NULL, ...) {
    #checkLeafLetVersion()
    ...
    if (is.lonlat(x) && ((e$ymin < -85) || (e$ymax > 85))) {
        ...
        yr2 <- e$ymax - e$ymin
        x <- spatSample(x[[y]], (yr1 / yr2) * maxcell, "regular",
    ...
    if (hasRGB) {
        ...
        leg <- .get_cls(x, type = type, dig.lab = 3, cols = col,
            breaks = breaks, breakby = breakby, ...)
        x <- leg$x
        col <- leg$cols
    }
    ...
    if (nlyr(x) == 1) {
        if (leg$type == "continuous") {
            ...
            map <- leaflet::addRasterImage(map, x, colors = pal,
                opacity = alpha, project = notmerc)
            if (!is.null(legend)) {
            ...
            map <- leaflet::addRasterImage(map, x, colors = col,
                opacity = alpha, project = notmerc)
            if (!is.null(legend)) {
        ...
        for (i in 1:nlyr(x)) {
            if (one_legend) {
                map <- leaflet::addRasterImage(map, x[[i]], colors = pal,
                    opacity = alpha, group = nms[i], project = notmerc)
            } else {
                if (hasColFun) {
                    pal <- col
                    range <- environment(col)$rng
                    ...
                    if (is.null(range)) {
                        ...
                        x[[i]] <- clamp(x[[i]], range[1, i], range[2, i],
                            values = TRUE)
                    }
        ...
        if (many_legends && add_legend) { # show one legend at a time
            map <- htmlwidgets::onRender(map, "function(el, x) {
...
setMethod("plet", signature(x = "SpatRasterCollection"),
    function (x, col, alpha = 0.8, main = names(x),
              tiles = c("Streets", "Esri.WorldImagery", "OpenTopoMap"),
              wrap = TRUE, maxcell = 500000, stretch = NULL,
              legend = "bottomright", type = NULL, breaks = NULL,
              breakby = "eqint", range = NULL, fill_range = FALSE,
              map = NULL, ...) {
    if (is.null(range)) {
