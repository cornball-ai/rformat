...
#' @keywords internal
#' @export
tinytest <- function (result, call, trace = NULL, diff = NA_character_,
                      short = c(NA_character_, "data", "attr", "xcpt", "envv",
                                "wdir", "file", "lcle"),
                      info = NA_character_, file = NA_character_,
                      fst = NA_integer_, lst = NA_integer_, ...) {
...
format.tinytest <- function (x, type = c("long", "short"), ...) {
    ...
    if (type == "short") {
        sprintf("%s: %s<%s--%s> %s", result, basename(file), fst, lst,
                oneline(call))
    } else {
        str <- sprintf(longfmt, result, file, fst, lst,
                       indent(call, with = " call| "))
        if (isFALSE(x) || is.na(x)) {
...
is_atomic <- function (x) {
    inherits(x, "POSIXct") || (
        length(class(x)) == 1 &&
        class(x) %in% c("character", "logical", "factor", "ordered", "integer",
                        "numeric", "complex")
    )
}
...
longdiff <- function (current, target, alt) {
    equivalent_data <- all.equal(target, current, check.attributes = FALSE,
                                 use.names = FALSE)

    if (identical(class(current), class(target)) && is_scalar(current) &&
        is_scalar(target)) {
...
#'
#' @export
expect_equal <- function (current, target,
    ...
    if (equal) {
        ...
        short <- shortdiff(current, target, tolerance = tolerance)
    }

    tinytest(result = equal, call = sys.call(sys.parent(1)), diff = diff,
...
expect_match <- function (current, pattern, info = NA_character_, ...) {
    ...
    diff <- if (out) {
        ...
        if (length(current) == 1) {
            sprintf("Expected string that matches '%s', got '%s'.", pattern,
                current)
        } else {
    ...
    tinytest(result = out, call = sys.call(sys.parent(1)), diff = diff,
        short = short, info = info)
}
...
#' @rdname expect_equal
#' @export
expect_equivalent <- function (current, target,
                               tolerance = sqrt(.Machine$double.eps),
                               info = NA_character_, ...) {
...
expect_true <- function (current, info = NA_character_) {
    ...
    if (!result) {
        ...
        else if (is.logical(current)) sprintf("'logical' of length %d",
                                              length(current))
...
expect_silent <- function (current, quiet = TRUE, info = NA_character_) {
    ...
    if (quiet) {
        ...
        if (has_nullfile) {
            dumpfile <- do.call("nullfile", list())
        } else {
            dumpfile <- tempfile()
        }
        sink(dumpfile)
...
expect_null <- function (current, info = NA_character_) {
    ...
    if (is.null(current)) {
        ...
        tinytest(FALSE, call = call, short = "data"
            , diff = sprintf("Expected NULL, got '%s'",
...
#' @export
expect_error <- function (current, pattern = ".*", class = "error",
                          info = NA_character_, ...) {
    result <- FALSE
    diff <- "No error"
...
#' @rdname expect_equal
#' @export
expect_warning <- function (current, pattern = ".*", class = "warning",
                            info = NA_character_, strict = FALSE, ...) {
    messages <- list()
    warnings <- list()
    errors <- list()
...
#' @rdname expect_equal
#' @export
expect_message <- function (current, pattern = ".*", class = "message",
                            info = NA_character_, strict = FALSE, ...) {
    messages <- list()
    warnings <- list()
    errors <- list()
