...
add_locally_masked_functions <- function (envir, output) {
    ...
    envir$expect_equal_to_reference <- capture(expect_equal_to_reference,
                                               output)
    envir$expect_equivalent_to_reference <- capture(expect_equivalent_to_reference,
                                                    output)
...
run_test_file <- function (file, at_home = TRUE,
                           ...
                           set_env = list(), encoding = "unknown", ...) {
    if (!file_test("-f", file)) {
        stop(sprintf("'%s' does not exist or is a directory", file),
             call. = FALSE)
    }
    ...
    if (length(set_env) > 0) {
        ...
        old_env_var <- sapply(names(set_env), Sys.getenv,
                              unset = NA_character_, USE.NAMES = TRUE)
        # new settings
    ...
    if (nchar(prfile) > 30) {
        prfile <- paste0("..",
                         substr(prfile, nchar(prfile) - 27, nchar(prfile)))
    }
...
"
    issues <- sprintf("> %s#%03d: %s", basename(filename), line_numbers,
        occurrences)
...
print_status <- function (filename, env, color, print = TRUE) {
    ...
    else sprintf(if (color) "\033[0;31m%d fails\033[0m" else "%d fails",
        env$nfail())

    side <- if (env$nside() == 0) ""
    else sprintf(if (color) "\033[0;93m%d side-effects\033[0m " else "%d side-effects ",
        env$nside())

    if (print) {
        cat(prefix, fails, side, sep = " ")
    } else {
        paste(prefix, fails, side, sep = " ")
    }
}
...
run_test_dir <- function (dir = "inst/tinytest", pattern = "^test.*\\.[rR]$",
                          ...
                          verbose = getOption("tt.verbose", 2),
                          color = getOption("tt.pr.color", TRUE),
                          remove_side_effects = TRUE, cluster = NULL,
    ...
    if (!inherits(cluster, "cluster")) {
        ...
        test_output <- lapply(basename(testfiles), run_test_file,
            at_home = at_home, verbose = verbose,
            color = color,
            remove_side_effects = remove_side_effects, ...)
    } else {
        parallel::clusterEvalQ(cluster, library(tinytest))
        test_output <- parallel::parLapply(cluster, testfiles, run_test_file,
            at_home = at_home,
...
test_package <- function (pkgname, testdir = "tinytest", lib.loc = NULL,
    ...
    if (any(i_fail) && !interactive()) {
        ...
        stop(sum(i_fail), " out of ", length(out), " tests failed",
             call. = FALSE)
    }
    out
}
...
build_install_test <- function (pkgdir = "./", testdir = "tinytest",
                                pattern = "^test.*\\.[rR]$", at_home = TRUE,
                                verbose = getOption("tt.verbose", 2),
                                color = getOption("tt.pr.color", TRUE),
                                ncpu = 1, remove_side_effects = TRUE,
                                side_effects = FALSE,
                                lc_collate = getOption("tt.collate", NA),
                                keep_tempdir = FALSE, encoding = "unknown") {
    oldwd <- getwd()
    tdir <- tempfile()
    on.exit({ setwd(oldwd)
        if (keep_tempdir) {
    ...
    ## build package
    system2("R",
            args = c("CMD", "build", "--no-build-vignettes", "--no-manual",
                     shQuote(pkg)))
...
"
    scr <- sprintf(script, pkgname, pattern, normalizePath(tdir, winslash = "/", mustWork = FALSE), testdir, at_home, verbose, color, remove_side_effects, side_effects, ncpu, lc_collate, encoding)
