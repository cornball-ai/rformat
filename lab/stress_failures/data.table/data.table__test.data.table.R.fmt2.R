test.data.table <- function (script = "tests.Rraw", verbose = FALSE,
                             pkg = ".", silent = FALSE,
                             showProgress = interactive() && !silent,
                             testPattern = NULL,
                             memtest = Sys.getenv("TEST_DATA_TABLE_MEMTEST",
                                                  0L),
                             memtest.id = NULL, optional = FALSE) {
    stopifnot(isTRUEorFALSE(verbose), isTRUEorFALSE(silent),
              isTRUEorFALSE(showProgress), isTRUEorFALSE(optional))
    ...
    if (length(memtest.id)) {
        if (length(memtest.id) == 1L) {
            memtest.id <- rep(memtest.id,
                              2L) # for convenience of supplying one id rather than always a range
        }
        ...
        if (memtest == 0L) {
            memtest <- 1L # using memtest.id implies memtest
        }
    }
    if (exists("test.data.table", .GlobalEnv, inherits = FALSE)) {
        ...
        # nocov start
        dev <- TRUE
        if ("package:data.table" %chin% search()) {
            stopf("data.table package is loaded. Unload or start a fresh R session.")
        }
        rootdir <- if (pkg != "." &&
            pkg %chin% dir()) file.path(getwd(),
                                        pkg) else Sys.getenv("PROJ_PATH",
                                                             normalizePath("."))
    ...
    stopifnot(is.character(script), length(script) == 1L, !is.na(script),
              nzchar(script))
    ...
    oldEnv <- Sys.getenv(c("_R_CHECK_LENGTH_1_LOGIC2_", "TZ"),
                         unset = NA_character_)
    ...
    if (is.null(options()$warnPartialMatchArgs)) {
        options(warnPartialMatchArgs = FALSE) # R 3.1.0 had a NULL default for these 3. Set to FALSE
    }
    if (is.null(options()$warnPartialMatchAttr)) {
        options(warnPartialMatchAttr = FALSE) # now otherwise options(oldOptions) fails later.
    }
    if (is.null(options()$warnPartialMatchDollar)) {
    ...
    oldOptions <- options(
        ...
        width = max(getOption('width'),
                    80L), # some tests (e.g. 1066, 1293) rely on capturing output that will be garbled with small width
    ...
    foreign <- gettext("object '%s' not found",
                       domain = "R") != "object '%s' not found"
    if (foreign) {
        # nocov start
    ...
    assign("lasttime", proc.time()[3L],
           envir = env) # used by test() to attribute time in between tests to the next test
    assign("timings",
           data.table(ID = seq_len(9999L), time = 0.0, nTest = 0L, RSS = 0.0),
           envir = env) # test timings aggregated to integer id
    ...
    if (memtest) {
        catf("\n***\n*** memtest=%d. This should be the first call in a fresh R_GC_MEM_GROW=0 R session for best results. Ctrl-C now if not.\n***\n\n",
             memtest)
        if (is.na(rss())) {
            stopf("memtest intended for Linux. Step through data.table:::rss() to see what went wrong.")
        }
    }
    ...
    if (!is.null(testPattern)) {
        ...
        test_call_args <- test_calls[pd[token == 'expr'], list(call_id = parent,
                                                               arg_line = i.line1,
                                                               col1,
                                                               col2), on = c(id = 'parent'), nomatch = NULL]
        ...
        keep_test_ids <- test_num_expr[grepl(testPattern,
                                             substring(file_lines[arg_line],
                                                       col1, col2)), call_id]
    ...
    cat("\n", date(), # so we can tell exactly when these tests ran on CRAN to double-check the result is up to date
        ...
        ", l10n_info()=='", paste0(names(l10n_info()), "=", l10n_info(),
                                   collapse = "; "), "'",
        ", getDTthreads()=='", paste(gsub("[ ][ ]+", "==", gsub("^[ ]+", "", capture.output(invisible(getDTthreads(verbose = TRUE))))),
                                     collapse = "; "), "'",
    ...
    if (inherits(err, "try-error")) {
        ...
        stopf("Failed in %s after test %s before the next test() call in %s",
              timetaken(env$started.at), env$prevtest, fn)
    ...
    if (nfail > 0L) {
        # nocov start
        stopf(ngettext(nfail, "%d error out of %d. Search %s for test number %s. Duration: %s.", "%d errors out of %d. Search %s for test numbers %s. Duration: %s."),
              nfail, ntest, names(fn), toString(env$whichfail),
              timetaken(env$started.at), domain = NA)
    ...
    if (length(env$warnings)) {
        ...
        catf(ngettext(nrow(warnings), "Caught %d warning outside the test() calls:\n", "Caught %d warnings outside the test() calls:\n"),
             nrow(warnings), domain = NA)
        print(warnings, nrows = nrow(warnings))
        stopf("Tests succeeded, but non-test code caused warnings. Search %s for tests shown above.",
              names(fn))
        # nocov end
    }
    ...
    if (!memtest) {
        ans <- head(timings[if (dev) - 1L else TRUE][order(-time)],
                    10L)[, RSS := NULL] # exclude id 1 in dev as that includes JIT
        ...
        catf("10 longest running tests took %ds (%d%% of %ds)\n",
             as.integer(tt <- ans[, sum(time)]),
             as.integer(100.0 * tt / (ss <- timings[, sum(time)])),
             as.integer(ss))
...
test <- function (num, x, y = TRUE, error = NULL, warning = NULL,
                  message = NULL, output = NULL, notOutput = NULL,
                  ignore.warning = NULL, options = NULL, env = NULL) {
    if (!is.null(env)) {
    ...
    string_match <- function (x, y, ignore.case = FALSE) {
        ...
        # try treating x as literal first; useful for most messages containing ()[]+ characters
    ...
    if (foreign && .test.data.table &&
        (length(error) || length(warning) || length(message) ||
            length(output) || length(notOutput) || length(ignore.warning)))
    ...
    if (!fail && !length(error) && (length(output) || length(notOutput))) {
        ...
        if (length(notOutput) && !foreign &&
            string_match(notOutput, out, ignore.case = TRUE)) {
            # nocov start
    ...
    if (!fail && !length(error) &&
        (!length(output) || !missing(y))) { # TODO test y when output=, too
        ...
        if (is.data.frame(x) && is.data.frame(y)) {
            if ((is.data.table(x) && !selfrefok(x)) ||
                ...
                if (identical(xc, yc) && identical(key(x), key(y))) {
                    return(invisible(TRUE)) # check key on original x and y because := above might have cleared it on xc or yc
                }
                if (isTRUE(all.equal.result <- all.equal(xc, yc, check.environment = FALSE)) && identical(key(x), key(y)) &&
                    ...
                    identical(vapply_1c(xc, typeof), vapply_1c(yc, typeof))) {
                    return(invisible(TRUE))
                }
            }
        }
        if (is.atomic(x) && is.atomic(y) &&
            isTRUE(all.equal.result <- all.equal(x, y, check.names = !isTRUE(y))) && typeof(x) == typeof(y)) {
            return(invisible(TRUE))
        }
