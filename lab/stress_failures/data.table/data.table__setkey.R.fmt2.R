setkey <- function (x, ..., verbose = getOption("datatable.verbose"),
                    physical = TRUE) {
    if (is.character(x)) {
        stopf("x may no longer be the character name of the data.table. The possibility was undocumented and has been removed.")
    }
...
setkeyv <- function (x, cols, verbose = getOption("datatable.verbose"),
    ...
    if (is.null(cols)) { # this is done on a data.frame when !cedta at top of [.data.table
        ...
        setattr(x, "index",
                NULL) # setkey(DT,NULL) also clears secondary keys. setindex(DT,NULL) just clears secondary keys.
        return(invisible(x))
    }
    if (!missing(verbose)) {
    ...
    if (!is.character(cols)) {
        stopf("cols is not a character vector. Please see further information in ?setkey.")
    }
    if (physical && .Call(C_islocked, x)) {
        stopf("Setting a physical key on .SD is reserved for possible future use; to modify the original data's order by group. Try setindex() instead. Or, set*(copy(.SD)) as a (slow) last resort.")
    }
    if (!length(cols)) {
    ...
    if (identical(cols, "")) {
        stopf("cols is the empty string. Use NULL to remove the key.")
    }
    if (!all(nzchar(cols))) { stopf("cols contains some blanks.") }
    cols <- gsub("`", "", cols, fixed = TRUE)
    miss <- !(cols %chin% colnames(x))
    if (any(miss)) {
        stopf("some columns are not in the data.table: %s",
              brackify(cols[miss]), class = "dt_missing_column_error")
    }

    if (physical &&
        identical(head(key(x), length(cols)),
                  cols)) { ## for !physical we need to compute groups as well #4387
    ...
    if (".xi" %chin% names(x)) {
        stopf("x contains a column called '.xi'. Conflicts with internal use by data.table.")
    }
    for (i in cols) {
        .xi <- x[[i]] # [[ is copy on write, otherwise checking type would be copying each column
        if (!typeof(.xi) %chin% ORDERING_TYPES) {
            stopf("Column '%s' is type '%s' which is not supported as a key column type, currently.",
                  i, typeof(.xi), class = "dt_unsortable_type_error")
        }
    }
    if (!is.character(cols) || length(cols) < 1L) {
        internal_error("'cols' should be character at this point") # nocov
    }
    ...
    if (length(o)) {
        setattr(x, "index",
                NULL) # TO DO: reorder existing indexes likely faster than rebuilding again. Allow optionally. Simpler for now to clear. Only when order changes.
        if (verbose) { last.started.at <- proc.time() }
        .Call(Creorder, x, o)
        if (verbose) { catf("reorder took %s\n", timetaken(last.started.at)) ; flush.console() }
    } else {
        if (verbose) {
            catf("x is already ordered by these columns, no need to call reorder\n")
        }
...
indices <- function (x, vectors = FALSE) {
    ...
    if (is.null(ans)) {
        return(ans) # otherwise character() gets returned by next line
    }
    ans <- gsub("^__", "",
                ans) # the leading __ is internal only, so remove that in result
    if (isTRUE(vectors))
...
getindex <- function (x, name) {
    ...
    ans <- attr(attr(x, 'index', exact = TRUE),
                paste0("__", name, collapse = ""), exact = TRUE)
    if (!is.null(ans) &&
        (!is.integer(ans) || (length(ans) != nrow(x) && length(ans) != 0L))) {
...
is.sorted <- function (x, by = NULL) {
    if (is.list(x)) {
        if (missing(by)) {
            by <- seq_along(x) # wouldn't make sense when x is a vector; hence by=seq_along(x) is not the argument default
        }
...
maybe_reset_index <- function (x, idx, cols) {
    ...
    setattr(attr(x, "index", exact = TRUE), paste0("__", cols, collapse = ""),
            idx)
    invisible(x)
}

ORDERING_TYPES <- c('logical', 'integer', 'double', 'complex', 'character')
forderv <- function (x, by = seq_along(x), retGrp = FALSE, retStats = retGrp,
                     sort = TRUE, order = 1L, na.last = FALSE,
                     reuseSorting = getOption("datatable.reuse.sorting", NA)) {
    if (is.atomic(x) ||
        is.null(x)) { # including forderv(NULL) which returns error consistent with base::order(NULL),
        if (!missing(by) && !is.null(by)) {
            stopf("x is a single vector, non-NULL 'by' doesn't make sense")
        }
        by <- NULL
    } else {
        if (!length(x)) {
            return(integer(0L)) # e.g. forderv(data.table(NULL)) and forderv(list()) return integer(0L))
        }
        by <- colnamesInt(x, by, check_dups = FALSE)
    }
    order <- as.integer(order) # length and contents of order being +1/-1 is checked at C level
    .Call(CforderReuseSorting, x, by, retGrp, retStats, sort, order, na.last,
          reuseSorting) # returns integer() if already sorted, regardless of sort=TRUE|FALSE
}

forder <- function (..., na.last = TRUE, decreasing = FALSE, method = "radix") {
    if (method != "radix") {
        stopf("data.table has no support for sorting by method='%s'. Use base::order(), not order(), if you really need this.",
              method)
    }
    stopifnot(is.logical(decreasing), length(decreasing) > 0L,
              !is.na(decreasing))
    sub <- substitute(list(...))
    tt <- vapply_1b(sub, function (x) is.null(x) ||
        (is.symbol(x) && !nzchar(x)))
    if (any(tt)) {
        sub[tt] <- NULL # remove any NULL or empty arguments; e.g. test 1962.052: forder(DT, NULL) and forder(DT, )
    }
    if (length(sub) < 2L) {
        return(NULL) # forder() with no arguments returns NULL consistent with base::order
    }
    asc <- rep.int(1L,
                   length(sub) - 1L) # ascending (1) or descending (-1) per column
    ...
    if (is.list(x)) {
        if (length(x) == 0L && is.data.frame(x)) {
            stopf("Attempting to order a 0-column data.table or data.frame.")
        }
        ...
        if (length(sub) == 1L) {
            ...
            if (!is.data.frame(x)) {
                stopf("The first item passed to [f]order is a plain list but there are more items. It should be a data.table or data.frame.")
            }
            asc <- asc[-1L]
    ...
    if (length(decreasing) > 1L) {
        if (any(asc < 0L)) {
            stopf("Mixing '-' with vector decreasing= is not supported.")
        }
        if (length(decreasing) != length(asc)) {
            stopf("decreasing= has length %d applied to sorting %d columns.",
                  length(decreasing), length(asc))
        }
    ...
    o <- forderv(data, seq_along(data), retGrp = FALSE, retStats = FALSE,
                 sort = TRUE, order = orderArg, na.last = na.last)
    if (!length(o) && length(data) >= 1L) {
        o <- seq_along(data[[1L]])
    } else {
        o
    }
...
fsort <- function (x, decreasing = FALSE, na.last = FALSE, internal = FALSE,
    ...
    if (typeof(x) == "double" && !decreasing && !(containsNAs <- anyNA(x))) {
        if (internal) {
            stopf("Internal code should not be being called on type double")
        }
    ...
    if (!internal) {
        if (typeof(x) != "double") {
            warningf("Input is not a vector of type double. New parallel sort has only been done for double vectors so far. Using one thread.")
        }
        if (decreasing) {
            warningf("New parallel sort has not been implemented for decreasing=TRUE so far. Using one thread.")
        }
        if (containsNAs) {
            warningf("New parallel sort has not been implemented for vectors containing NA values so far. Using one thread.")
        }
    }
...
setorder <- function (x, ..., na.last = FALSE) # na.last=FALSE here, to be consistent with data.table's default
# as opposed to DT[order(.)] where na.last=TRUE, to be consistent with base
{
    ...
    if (length(cols)) {
        ...
        for (i in seq_along(cols)) {
            v <- as.list(cols[[i]])
            if (length(v) > 1L && v[[1L]] == "+") { v <- v[[-1L]] } else
            if (length(v) > 1L && v[[1L]] == "-") {
                v <- v[[-1L]]
                order[i] <- -1L
            }
...
setorderv <- function (x, cols = colnames(x), order = 1L, na.last = FALSE) {
    ...
    if (is.na(na.last) || !length(na.last)) {
        stopf('na.last must be logical TRUE/FALSE')
    }
    if (!is.character(cols)) {
        stopf("cols is not a character vector. Please see further information in ?setorder.")
    }
    if (!length(cols)) {
    ...
    if (!all(nzchar(cols))) {
        stopf("cols contains some blanks.") # TODO: probably I'm checking more than necessary here.. there are checks in 'forderv' as well
    }
    ...
    if (".xi" %chin% colnames(x)) {
        stopf("x contains a column called '.xi'. Conflicts with internal use by data.table.")
    }
    for (i in cols) {
        .xi <- x[[i]] # [[ is copy on write, otherwise checking type would be copying each column
        if (!typeof(.xi) %chin% ORDERING_TYPES) {
            stopf("Column '%s' is type '%s' which is not supported for ordering currently.",
                  i, typeof(.xi), class = "dt_unsortable_type_error")
        }
    }
    if (!is.character(cols) || length(cols) < 1L) {
    ...
    if (length(o)) {
        ...
        k <- key(x)
        if (!identical(head(cols, length(k)), k) ||
            any(head(order, length(k)) < 0L))
        setattr(x, 'sorted',
                NULL) # if 'forderv' is not 0-length & key is not a same-ordered subset of cols, it means order has changed. So, set key to NULL, else retain key.
        setattr(x, 'index',
                NULL) # remove secondary keys too. These could be reordered and retained, but simpler and faster to remove
    }
    invisible(x)
}
...
CJ <- function (..., sorted = TRUE, unique = FALSE) {
    ...
    for (i in seq_along(l)) {
        ...
        if (sorted) {
            if (!is.atomic(y)) {
                stopf("'sorted' is TRUE but element %d is non-atomic, which can't be sorted; try setting sorted = FALSE",
                      i)
            }
