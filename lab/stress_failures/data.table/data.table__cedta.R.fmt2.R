...
cedta.pkgEvalsUserCode <- c("gWidgetsWWW", "statET", "FastRWeb", "slidify",
                            "rmarkdown", "knitr", "ezknitr", "IRkernel", "rtvs")
...
.any_eval_calls_in_stack <- function () {
    ...
    for (ii in length(calls):1) { # nolint: seq_linter. rev(seq_len(length(calls)))? See https://bugs.r-project.org/show_bug.cgi?id=18406.
        ...
        if (is.name(the_call) && (the_call %chin% c("eval", "evalq"))) {
            return(TRUE)
        }
    }
    FALSE
...
cedta <- function (n = 2L) {
    ...
    # most common and recommended cases first for speed
    (nsname == "utils" &&
        (exists("debugger.look", parent.frame(n + 1L)) ||
            (length(sc) >= 8L && sc[[length(sc) - 7L]] %iscall% 'example'))) ||
    # 'example' for #2972
    (nsname == "base" && # lapply
