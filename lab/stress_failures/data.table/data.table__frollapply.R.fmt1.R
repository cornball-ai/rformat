...
simplifylist <- function (x, fill, ansmask) {
    ...
    if (length(all.ut) > 1L) {
        ...
        if (
            length(ans.ut) == 2L &&
            ## simplifylist(list(NA, 1, 1L, TRUE), NA, ansmask=c(F,T,T,T))
            all(c("double", "integer") %in% ans.ut) &&
            ## simplifylist(list(NA, 1, TRUE), NA, ansmask=c(F,T,T))
            equal.lengths(x[ansmask]) ## simplifylist(list(NA, 1, 1:2), NA, ansmask=c(F,T,T))
        ) {
            x[ansmask & all.t == "integer"] <- lapply(x[ansmask &
                    all.t == "integer"], as.numeric) ## coerce integer to double
            ans.ut <- "double"
        }
        ...
        if (
            length(ans.ut) == 1L &&
            ## simplifylist(list(NA, 1, TRUE), NA, ansmask=c(F,T,T))
            equal.lengths(x[ansmask]) &&
            ## simplifylist(list(NA, 1L, 1:2), NA, ansmask=c(F,T,T))
            ...
            if (identical(fill, NA)) {
                if (ans.ut == "list") {
                    fill <- lapply(ans1, `[`,
                                   NA) ## we want list(NA) rather than list(NULL), this also propagates names
                } else {
                ...
                fill.t != ans.ut &&
                ## simplifylist(list(-1, 1, 2), -1, ansmask=c(F,T,T))
                fill.t %in% c("double", "integer") &&
                ## simplifylist(list(NULL, 1, 2), NULL, ansmask=c(F,T,T))
                ans.ut %in% c("double",
                              "integer") ## simplifylist(list(1, "a", "b"), 1, ansmask=c(F,T,T))
            ) { ## fill=-2, ans=1L
            ...
            if (
                ...
                is.null(names(fill)) &&
                !is.null(names(ans1)) &&
                ## simplifylist(list(NA, c(1,2), c(1,2)), NA, ansmask=c(F,T,T))
    ...
    if (!is.null(names(fill)) && all_NULL_names(x[ansmask]) &&
        equal.lengths(x)) {
        nm <- names(fill)
    ...
    if (length(all.ut) == 1L) {
        if (all.ut %in% c("integer", "logical", "double", "complex", "character", "raw")) {
            ...
            } else if (equal.lengths(x) &&
                equal.names(x)) { ## length 2+ and equal
                return(rbindlist(lapply(x, as.list)))
            }
        } else if (identical(all.ut, "list")) {
            if (
                all_data.frame(x) &&
                ## simplifylist(list(NA, list(a=1L, b=2L), data.table(a=1L, b=2L)), NA, ansmask=c(F,T,T))
                equal.dims(x) &&
                ## simplifylist(list(NA, data.table(a=1L, b=2L), data.table(a=1L)), NA, ansmask=c(F,T,T))
                equal.types(x) &&
                ## simplifylist(list(NA, data.table(a=1L, b=2L), data.table(a=1L, b="b")), NA, ansmask=c(F,T,T))
                equal.names(x) ## simplifylist(list(NA, data.table(a=1L, b=2L), data.table(x=1L, y=2L)), NA, ansmask=c(F,T,T))
            ) {
                return(rbindlist(x))
            } else if (
                equal.lengths(x) &&
                len_unq(lapply(x, lengths, use.names = FALSE)) <= 1L &&
                ## nested lengths
...
fixselfref <- function (x) {
    if (length(x) &&
        is.data.table(x[[1L]])) { ## calling is.data.table many times always may be too much, so first we check only first element
        dtmask <- vapply_1b(x, is.data.table,
                            use.names = FALSE) ## most likely all, but we cannot be sure that function is type stable
        x[dtmask] <- lapply(x[dtmask], setalloccol)
    }
    x
}

frollapply <- function (X, N, FUN, ..., by.column = TRUE, fill = NA,
                        align = c("right", "left", "center"),
                        adaptive = FALSE, partial = FALSE,
    ...
    if (leftadaptive) {
        ...
        if (by.column) {
            ...
            rev.d <- function (d) {
                l <- lapply(d, rev)
                if (is.data.table(d)) { setDT(l) } else
                if (is.data.frame(d)) { setDF(l) } else { l }
            }
            X <- lapply(X, rev.d)
        }
    ...
    if (!adaptive) {
        ...
        tight0 <- function (i, dest, src, n) FUN(dest,
                                                 ...) ## skip memcpy when n==0
        if (by.column) {
        ...
        if (by.column) {
            ...
            tight <- function (i, dest, src, n) FUN(.Call(CmemcpyVectoradaptive, dest, src, i, n),
                                                    ...) # CmemcpyVectoradaptive handles k[i]==0
        } else {
            if (!list.df) {
            ...
            tight <- function (i, dest, src, n) FUN(.Call(CmemcpyDTadaptive, dest, src, i, n),
                                                    ...) # CmemcpyDTadaptive handles k[i]==0
        }
    }
    ...
    for (i in seq_len(nx)) {
        ...
        for (j in seq_len(nn)) {
            if (!thislen) {
                ans[[(i - 1L) * nn + j]] <- vector(mode = typeof(thisx),
                                                   length = 0L)
                next
            }
            thisn <- N[[j]]
            ...
            ansi <- which(ansmask)
            tightFUN <- if (adaptive ||
                thisn) { ## handle n==0 for !adaptive, for !adaptive thisn should be scalar
                tight
            } else {
                tight0
            }
            if (use.fork) { ## !windows && getDTthreads()>1L, and then throttle using getDTthreadsC(thislen, TRUE)
                ...
                ii <- split(ansi,
                            sort(rep_len(seq_len(ths), length(ansi)))) ## assign row indexes to threads
                ...
                if (length(ansi)) {
                    fork.res <- withCallingHandlers( ## collect results early to minimize time when user could raise SIGINT
                        tryCatch(
                            ...
                            error = function (e) stopf(err.collect,
                                                       e[["message"]]),
                            warning = function (w) {
                    ...
                    fork.err <- vapply_1b(fork.res, inherits, "try-error",
                                          use.names = FALSE)
                    if (any(fork.err)) {
                        stopf(
                            "frollapply received an error(s) when evaluating FUN:\n%s",
                            attr(fork.res[fork.err][[1L]], "condition",
                                 TRUE)[["message"]] ## print only first error for consistency to single threaded code
                        )
                    }
                    thisans <- unlist(fork.res, recursive = FALSE,
                                      use.names = FALSE)
                ...
                withCallingHandlers(
                    tryCatch(
                        thisans <- lapply(ansi, FUN = tightFUN, dest = cpy(w),
                                          src = thisx, n = thisn),
            ...
            if (leftadaptive) {
                ...
                ansmask <- shift(ansmask,
                                 if (align == "left") 1L - thisn else -floor(thisn / 2L),
                                 fill = FALSE, type = "shift")
                ansi <- which(ansmask)
            }
            ## fill
            thisansi <- (i - 1L) * nn + j
            ...
            if (is.function(simplify)) {
                ans[[thisansi]] <- tryCatch(
                    ...
                    warning = function (w) warningf(warn.simplify,
                                                    w[["message"]])
                )
            } else if (isTRUE(simplify)) {
                ans[[thisansi]] <- tryCatch(
    ...
    if (length(ans)) {
        if (!xvec && length(ans) == 1L) {
            ...
            nms <- make.roll.names(x.len = nx, n.len = nn, n = orig$N,
                                   x.nm = xnam, n.nm = nnam, fun = "apply",
