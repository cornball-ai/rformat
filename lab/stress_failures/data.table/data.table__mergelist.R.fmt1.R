...
fdistinct <- function (x, on = key(x), mult = c("first", "last"),
                       cols = seq_along(x), copy = TRUE) {
    if (!perhaps.data.table(x))
    stopf("'x' must be data.table")
    if (!is.character(on) || !length(on) || anyNA(on) ||
        !all(on %chin% names(x)))
    stopf("'on' must be character column names of 'x' argument")
    mult <- match.arg(mult)
    if (is.null(cols))
    cols <- seq_along(x)
    else if (!(is.character(cols) || is.integer(cols)) || !length(cols) ||
        anyNA(cols))
    ...
    if (mult == "last") {
        if (!sort) {
            internal_error("sort must be TRUE when computing mult='last'") # nocov
        }
        f <- c(f[-1L] - 1L, nrow(x)) ## last of each group
    }
    if (length(o)) { f <- o[f] }
    if (sort && length(o <- forderv(f))) {
        f <- f[o] ## this rolls back to original order
    }
...
dtmerge <- function (x, i, on, how, mult, join.many, void = FALSE, verbose) {
    nomatch <- switch(how, inner =, semi =, anti =, cross = 0L, left =,
                      right =, full = NA_integer_)
    nomatch0 <- identical(nomatch, 0L)
    if (is.null(mult))
    mult <- switch(how, semi =, anti = "last", cross = "all", inner =, left =,
                   right =, full = "error")
    if (void && mult != "error")
    ...
    if (how == "cross") { ## short-circuit bmerge results only for cross join
        ...
        ans <- list(allLen1 = FALSE, starts = rep.int(1L, nrow(i)),
                    lens = rep.int(nrow(x), nrow(i)), xo = integer())
    } else {
        if (!length(on))
        ...
        } else if (mult == "all" && !ans$allLen1 && !join.many &&
            ## join.many, like allow.cartesian, check
            !(length(ans$starts) == 1L && ans$lens == nrow(x)) &&
...
mergepair <- function (lhs, rhs, on, how, mult, lhs.cols = names(lhs),
                       rhs.cols = names(rhs), copy = TRUE, join.many = TRUE,
                       verbose = FALSE) {
    ...
    if (how != "cross") {
        if (is.null(on)) {
            if (how == "left" || semi_or_anti) { on <- key(rhs) } else
            if (how == "right") { on <- key(lhs) } else
            if (inner_or_full) { on <- onkeys(key(lhs), key(rhs)) }
            if (is.null(on))
            stopf("'on' is missing and necessary key is not present")
        }
        if (any(bad.on <- !on %chin% names(lhs)))
        stopf("'on' argument specifies columns to join %s that are not present in %s table %s",
              brackify(on[bad.on]), "LHS", brackify(names(lhs)))
        if (any(bad.on <- !on %chin% names(rhs)))
        stopf("'on' argument specifies columns to join %s that are not present in %s table %s",
              brackify(on[bad.on]), "RHS", brackify(names(rhs)))
    } else if (is.null(on)) {
    ...
    if (inner_or_full && !is.null(mult) &&
        (mult == "first" || mult == "last")) {
        join_from <- fdistinct(join_from, on = on, mult = mult,
                               cols = from_cols,
                               copy = FALSE) ## might not copy when already unique by 'on'
        copy_i <- nrow(join_from) != nrow(lhs) ## nrow(lhs) bc how='inner|full' so join_from=lhs
    } else if (how == "inner" &&
        (is.null(mult) ||
            mult == "error")) { ## we do this branch only to raise error from bmerge, we cannot use forder to just find duplicates because those duplicates might not have matching rows in another table, full join checks mult='error' during two non-void bmerges
        dtmerge(x = join_from, i = join_to, on = on, how = how, mult = mult,
                verbose = verbose, join.many = join.many, void = TRUE)
    }

    ## binary merge
    ans <- dtmerge(x = join_to, i = join_from, on = on, how = how, mult = mult,
                   verbose = verbose, join.many = join.many)

    ## make i side; avoid subsetting if possible
    cols_i <- someCols(join_from, from_cols, keep = on,
                       retain.order = semi_or_anti)
    ...
    if (semi_or_anti) {
        ...
        out.x <- .Call(CsubsetDT, join_to, ans$xrows,
                       someCols(join_to, to_cols, drop = on))
        copy_x <- TRUE
        ## ensure no duplicated column names in merge results
        if (any(dup.i <- names(out.i) %chin% names(out.x)))
        stopf("merge result has duplicated column names %s, use 'cols' argument or rename columns in 'l' tables",
              brackify(names(out.i)[dup.i]))
    }
    ...
    if (how != "full") {
        ...
        if (how == "right") {
            setcolorder(out,
                        neworder = c(on,
                                     names(out.x))) ## arrange columns: i.on, x.cols, i.cols
        }
    } else { # how=="full"
        ...
        supplement_rows <- dtmerge(x = join_to, i = join_from, on = on,
                                   how = "anti", mult = mult,
                                   verbose = verbose,
                                   join.many = join.many)$irows

        ## make anti join side
        cols_r <- someCols(join_from, from_cols, keep = on)
        if (is.null(supplement_rows)) {
            out.r <- .shallow(join_from, cols = cols_r,
                              retain.key = TRUE) ## retain.key is used only in the edge case when !nrow(out.i)
        } else {
        ...
        if (!nrow(out.r)) { ## possibly also !nrow(out.i)
            ...
            if (length(add <- setdiff(names(out.i), names(out.r)))) { ## add missing columns of proper types NA
                ...
                out.i <- lapply(unclass(out.i)[add], `[`,
                                seq_len(nrow(out.r))) ## could eventually remove this when cbindlist recycle 0 rows up, note that we need out.r not to be copied
...
mergelist_impl_ <- function (l, on, cols, how, mult, join.many, copy) {
    ...
    stopf("Tables in 'l' must all have columns, but these entries have 0: %s",
          brackify(which(!idx)))
    if (any(idx <- vapply_1i(l, function (x) anyDuplicated(names(x))) > 0L))
    stopf("Column names in individual 'l' entries must be unique, but these have some duplicates: %s",
          brackify(which(idx)))
    ...
    if (length(join.many) != n - 1L ||
        !all(vapply_1b(join.many, isTRUEorFALSE)))
    ...
    if (length(mult) != n - 1L ||
        !all(vapply_1b(mult, function (x) is.null(x) || (is.character(x) && length(x) == 1L && !anyNA(x) && x %chin% c("error", "all", "first", "last")))))
    ...
    if (length(how) != n - 1L ||
        !all(vapply_1b(how, function (x) is.character(x) && length(x) == 1L && !anyNA(x) && x %chin% c("left", "inner", "full", "right", "semi", "anti", "cross"))))
    ...
    if (missing(on) || is.null(on)) {
        ...
        on <- rep(list(on), n - 1L)
        if (length(on) != n - 1L ||
            !all(vapply_1b(on, function (x) is.character(x) && !anyNA(x) && !anyDuplicated(x)))) { ## length checked in dtmerge
    ...
    .Call(CcopyCols, out,
          colnamesInt(out,
                      names(out.mem)[out.mem %chin% unique(unlist(l.mem, recursive = FALSE))]))
    if (verbose)
    catf("mergelist: merging %d tables, took %.3fs\n", n, proc.time()[[3L]] - p)
    out
}

mergelist <- function (l, on, cols = NULL,
                       how = c("left", "inner", "full", "right", "semi",
                               "anti", "cross"),
...
setmergelist <- function (l, on, cols = NULL,
                          how = c("left", "inner", "full", "right", "semi",
                                  "anti", "cross"),
                          mult, join.many = getOption("datatable.join.many")) {
    if (missing(how) || is.null(how))
