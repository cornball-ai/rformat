fread <- function (input = "", file = NULL, text = NULL, cmd = NULL,
                   sep = "auto", sep2 = "auto", dec = "auto", quote = "\"",
                   ...
                   blank.lines.skip = FALSE, comment.char = "", key = NULL,
                   index = NULL,
                   ...
                   keepLeadingZeros = getOption("datatable.keepLeadingZeros",
                                                FALSE),
                   yaml = FALSE, tmpdir = tempdir(), tz = "UTC") {
    if (missing(input) + is.null(file) + is.null(text) + is.null(cmd) < 3L) {
        stopf("Used more than one of the arguments input=, file=, text= and cmd=.")
    }
    ...
    if (is.null(sep)) {
        ...
        if (sep == "") { sep <- "\n" } # meaning readLines behaviour. The 3 values (NULL, "" or "\n") are equivalent.
            ...
            sep <- "" # sep=="" at C level means auto sep
        } else {
            stopifnot(nchar(sep) == 1L) # otherwise an actual character to use as sep
        }
    }
    ...
    if (length(encoding) != 1L ||
        !encoding %chin% c("unknown", "UTF-8", "Latin-1")) {
    ...
    stopifnot(isTRUEorFALSE(strip.white), isTRUEorFALSE(blank.lines.skip),
              isTRUEorFALSE(fill) || is.numeric(fill) && length(fill) == 1L &&
              fill >= 0L,
              isTRUEorFALSE(showProgress), isTRUEorFALSE(verbose),
              isTRUEorFALSE(check.names), isTRUEorFALSE(logical01),
              isTRUEorFALSE(logicalYN), isTRUEorFALSE(keepLeadingZeros),
              isTRUEorFALSE(yaml),
              isTRUEorFALSE(stringsAsFactors) || (is.double(stringsAsFactors) &&
                  length(stringsAsFactors) == 1L && 0.0 <= stringsAsFactors &&
                  stringsAsFactors <= 1.0),
              is.numeric(nrows), length(nrows) == 1L)
    if (!is.character(comment.char) || length(comment.char) != 1L ||
        is.na(comment.char) || nchar(comment.char) > 1L) {
    ...
    if (is.na(nrows) || nrows < 0L) {
        nrows <- Inf # accept -1 to mean Inf, as read.table does
    }
    ...
    if (!is.null(text)) {
        if (!is.character(text)) {
            stopf("'text=' is type %s but must be character.", typeof(text))
        }
        if (!length(text)) { return(data.table()) }
        if (length(text) > 1L) {
            writeLines(text,
                       tmpFile <- tempfile(tmpdir = tmpdir)) # avoid paste0() which could create a new very long single string in R's memory
            file <- tmpFile
        ...
        if (input == "" || length(grep('\\n|\\r', input))) {
            ...
            !file.exists(input)) { # file name or path containing spaces is not a command
            cmd <- input
            if (input_has_vars &&
                getOption("datatable.fread.input.cmd.message", TRUE)) {
    ...
    if (!is.null(cmd)) {
        ...
        if (status != 0) {
            stopf("External command failed with exit code %d. This can happen when the disk is full in the temporary directory ('%s'). See ?fread for the tmpdir argument.",
                  status, tmpdir)
        }
        file <- tmpFile
    }
    if (!is.null(file)) {
        ...
        if (w <- startsWithAny(file, c("https://", "ftps://", "http://", "ftp://", "file://"))) { # avoid grepl() for #2531
            # nocov start
            tmpFile <- tempfile(fileext = paste0(".", tools::file_ext(file)),
                                tmpdir = tmpdir) # retain .gz extension in temp filename so it knows to be decompressed further below
            method <- if (w == 5L) "internal" # force 'auto' when file: to ensure we don't use an invalid option (e.g. wget), #1668
            else getOption("download.file.method",
                           default = "auto") # http: or ftp:
            # In text mode on Windows-only, R doubles up \r to make \r\r\n line endings. mode="wb" avoids that. See ?connections:"CRLF"
            download.file(file, tmpFile, method = method, mode = "wb",
                          quiet = !showProgress)
            file <- tmpFile
            on.exit(unlink(tmpFile), add = TRUE)
            # nocov end
        ...
        if (is.na(file_info$size)) {
            stopf("File '%s' does not exist or is non-readable. getwd()=='%s'",
                  file, getwd())
        }
        if (isTRUE(file_info$isdir)) {
            stopf("File '%s' is a directory. Not yet implemented.",
                  file) # Could use dir.exists(), but we already ran file.info().
        }
        if (!file_info$size) {
            warningf("File '%s' has size 0. Returning a NULL %s.", file,
        ...
        if ((w <- endsWithAny(file, c(".zip", ".tar"))) ||
            is_zip(file_signature)) {
        ...
        gzsig <- FALSE
        if ((w <- endsWithAny(file, c(".gz", ".bgz", ".bz2"))) ||
            (gzsig <- is_gzip(file_signature)) || is_bzip(file_signature)) {
            if (!requireNamespace("R.utils", quietly = TRUE))
            stopf("To read %s files directly, fread() requires 'R.utils' package which cannot be found. Please install 'R.utils' using 'install.packages('R.utils')'.",
                  if (w <= 2L || gzsig) "gz" else "bz2") # nocov
            # not worth doing a behavior test here, so just use getRversion().
            if (packageVersion("R.utils") < "2.13.0" &&
                base::getRversion() >= "4.5.0")
            ...
            on.exit(unlink(decompFile), add = TRUE)
            tryCatch({
                R.utils::decompressFile(file, decompFile, ext = NULL,
                                        FUN = FUN,
                                        remove = FALSE) # ext is not used by decompressFile when destname is supplied, but isn't optional
            }, error = function (e) {
                stopf("R.utils::decompressFile failed to decompress file '%s':\n  %s\n. This can happen when the disk is full in the temporary directory ('%s'). See ?fread for the tmpdir argument.",
                      file, conditionMessage(e), tmpdir)
                })
    ...
    if (is.logical(colClasses)) {
        if (!allNA(colClasses)) {
            stopf("colClasses is type 'logical' which is ok if all NA but it has some TRUE or FALSE values in it which is not allowed. Please consider the drop= or select= argument instead. See ?fread.")
        }
        colClasses <- NULL
    }
    if (!is.null(colClasses) &&
        is.atomic(colClasses)) { ## future R can use  if (is.atomic(.))
        if (!is.character(colClasses)) {
            stopf("colClasses is not type list or character vector")
        }
    ...
    if (yaml) {
        ...
        if ('header' %chin% yaml_names) {
            if ('header' %chin% call_args) {
                messagef("User-supplied 'header' will override that found in metadata.")
            } else {
                header <- as.logical(yaml_header$header)
            }
        }
        ...
        if (any(sep_idx <- sep_syn %chin% yaml_names)) {
            if ('sep' %chin% call_args) {
                messagef("User-supplied 'sep' will override that found in metadata.")
            } else {
                sep <- yaml_header[[sep_syn[sep_idx][1L]]]
            }
        }
        quote_syn <- c('quote', 'quoteChar', 'quote_char')
        if (any(quote_idx <- quote_syn %chin% yaml_names)) {
            if ('quote' %chin% call_args) {
                messagef("User-supplied 'quote' will override that found in metadata.")
            } else {
                quote <- yaml_header[[quote_syn[quote_idx][1L]]]
            }
        }
        dec_syn <- c('dec', 'decimal')
        if (any(dec_idx <- dec_syn %chin% yaml_names)) {
            if ('dec' %chin% call_args) {
                messagef("User-supplied 'dec' will override that found in metadata.")
            } else {
                dec <- yaml_header[[dec_syn[dec_idx][1L]]]
            }
        }
        if ('na.strings' %chin% yaml_names) {
            if ('na.strings' %chin% call_args) {
                messagef("User-supplied 'na.strings' will override that found in metadata.")
            } else {
                na.strings <- yaml_header$na.strings
            }
        }
    ...
    if (tz == "") {
        ...
        if (identical(tt, "") ||
            is_utc(tt)) { # empty TZ env variable ("") means UTC in C library, unlike R; _unset_ TZ means local
            tz <- "UTC"
        }
    }
    ...
    for (j in which(nzchar(colClassesAs))) { # # 1634
        ...
        if (new_class %chin% c("POSIXct")) {
            v[!nzchar(v)] <- NA_character_ # as.POSIXct/as.POSIXlt cannot handle as.POSIXct("") correctly #6208
        }
        new_v <- tryCatch({ # different to read.csv; i.e. won't error if a column won't coerce (fallback with warning instead)
            ...
            error = fun)
        set(ans, j = j,
            value = new_v) # aside: new_v == v if the coercion was aborted
    }
    ...
    if (yaml) {
        setattr(ans, 'yaml_metadata', yaml_header) # nocov
    }
