...
CoordPolar <- ggproto("CoordPolar", Coord,
    ...
    setNames(list(panel_params$theta.range, panel_params$r.range),
        c(self$theta, self$r))
    },
    ...
    details <- list(x.range = ret$x$range, y.range = ret$y$range,
        x.major = ret$x$major, y.major = ret$y$major, x.minor = ret$x$minor,
        y.minor = ret$y$minor, x.labels = ret$x$labels,
        y.labels = ret$y$labels, x.sec.range = ret$x$sec.range,
        y.sec.range = ret$y$sec.range, x.sec.major = ret$x$sec.major,
        y.sec.major = ret$y$sec.major, x.sec.minor = ret$x$sec.minor,
        y.sec.minor = ret$y$sec.minor, x.sec.labels = ret$x$sec.labels,
    ...
    setup_panel_guides = function (self, panel_params, guides,
                                   params = list()) {
    guide_names <- intersect(names(guides$guides),
        c("x", "x.sec", "y", "y.sec", "r", "r.sec", "theta", "theta.sec"))
    if (length(guide_names) > 0) {
        cli::cli_warn("{.fn {snake_class(self)}} cannot render {cli::qty(guide_names)} \\
        guide{?s} for the aesthetic{?s}: {.and {.field {guide_names}}}.")
    }
    panel_params
    },

    train_panel_guides = function (self, panel_params, layers,
                                   default_mapping, params = list()) {
    panel_params
    },
    ...
    ggname("grill", grobTree(
            ...
            element_render(
                ...
                x = rep(rfine,
                    each = length(thetafine)) * rep(sin(thetafine),
                        length(rfine)) + 0.5,
                y = rep(rfine,
                    each = length(thetafine)) * rep(cos(thetafine),
                        length(rfine)) + 0.5,
    ...
    theta <- theta_rescale(panel_params$theta.major, panel_params$theta.range,
        arc, dir)
...
theta_rescale_no_clip <- function (x, range, arc = c(0, 2 * pi),
                                   direction = 1) {
