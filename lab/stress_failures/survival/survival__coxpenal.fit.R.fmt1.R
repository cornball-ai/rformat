...
coxpenal.fit <- function (x, y, strata, offset, init, control,
    ...
    eps <- control$eps
    n <- nrow(y)
    if (is.matrix(x)) { nvar <- ncol(x) } else
    if (length(x) == 0) { stop("Must have an X variable") } else { nvar <- 1 }

    if (missing(offset) || is.null(offset)) { offset <- rep(0, n) }
    if (missing(weights) || is.null(weights)) { weights <- rep(1, n) } else {
    ...
    if (ncol(y) == 3) {
        if (length(strata) == 0) {
            sorted <- cbind(order(-y[, 2], y[, 3]), order(-y[, 1])) - 1L
            newstrat <- as.integer(n)
        } else {
            sorted <- cbind(order(strata, -y[, 2], y[, 3]), order(strata, -y[, 1])) - 1L
    ...
    for (i in 1:npenal) {
        temp <- unlist(lapply(assign, function (x, y)(length(x) == length(y) &&
                    all(x == y)), pcols[[i]]))
        if (sparse[i]) { pterms[temp] <- 2 } else { pterms[temp] <- 1 }
    ...
    if (any(sparse)) {
        ...
        f.expr1 <- function (coef) {
            ...
            if (is.null(extra1)) {
                ...
                temp <- pfun1(coef, theta1, n.eff, extra1)
            }
        ...
        coxlist1 <- list(coef = double(nfrail), first = double(nfrail), second = double(nfrail), penalty = 0.0, flag = FALSE)
    ...
    if (sum(!sparse) > 0) {
        ...
        debug(f.expr2)
        if (full.imat) {
            coxlist2 <- list(coef = double(nvar), first = double(nvar), second = double(nvar * nvar), penalty = 0.0, flag = rep(FALSE, nvar))
            length2 <- c(nvar, nvar, nvar * nvar, 1, nvar)
        } else {
            coxlist2 <- list(coef = double(nvar), first = double(nvar), second = double(nvar), penalty = 0.0, flag = rep(FALSE, nvar))
    ...
    #
    if (nfrail > 0) { finit <- rep(0, nfrail) } else { finit <- 0 }
    ...
    temp2 <- c('frailx', 'coxfit$fcoef', 'loglik1', 'coxfit$loglik', 'status', 'n.eff')
    temp3 <- c('xx[,pen.col]', 'coxfit$coef[pen.col]', 'loglik1', 'coxfit$loglik', 'status', 'n.eff')
    calls <- vector('expression', length(cfun))
    cargs <- lapply(pattr, function (x) x$cargs)
    for (i in 1:length(cfun)) {
        tempchar <- paste("(cfun[[", i, "]])(parmlist[[", i, "]], iter,", "iterlist[[", i, "]]")
        ...
        calls[i] <- parse(text = paste(tempchar, ")"))
            ...
            if (sparse[i]) {
                ...
                temp4 <- paste(temp3b[temp], collapse = ',')
            }
    ...
    else coxfit <- .C(Ccoxfit5a, as.integer(n), as.integer(nvar), y, xx, offset, weights, newstrat, sorted, means = double(nvar), coef = as.double(init), u = double(nvar), loglik = double(1), as.integer(method == 'efron'), as.integer(ptype), as.integer(full.imat), as.integer(nfrail), as.integer(frailx), f.expr1, f.expr2, rho, ifelse(zero.one, 0L, 1L))
    ...
    for (outer in 1:control$outer.max) {
        if (andersen) {
            coxfit <- .C(Cagfit5b, iter = as.integer(control$iter.max), as.integer(n), as.integer(nvar), as.integer(newstrat), coef = as.double(init), u = double(nvar + nfrail), hmat = double(nvar * (nvar + nfrail)), hinv = double(nvar * (nvar + nfrail)), loglik = double(1), flag = integer(1), as.double(control$eps), as.double(control$toler.chol), as.integer(method == 'efron'), as.integer(nfrail), fcoef = as.double(finit), fdiag = double(nfrail + nvar), f.expr1, f.expr2, rho)
        } else {
            coxfit <- .C(Ccoxfit5b, iter = as.integer(control$iter.max), as.integer(n), as.integer(nvar), as.integer(newstrat), coef = as.double(init), u = double(nvar + nfrail), hmat = double(nvar * (nvar + nfrail)), hinv = double(nvar * (nvar + nfrail)), loglik = double(1), flag = integer(1), as.double(control$eps), as.double(control$toler.chol), as.integer(method == 'efron'), as.integer(nfrail), fcoef = as.double(finit), fdiag = double(nfrail + nvar), f.expr1, f.expr2, rho)
        }
        ...
        if (need.df) {
            ...
            if (nfrail > 0) { temp1 <- coxlist1$second } else { temp1 <- 0 }
            if (ptype > 1) { temp2 <- coxlist2$second } else { temp2 <- 0 }

            dftemp <- coxpenal.df(matrix(coxfit$hmat, ncol = nvar), matrix(coxfit$hinv, ncol = nvar), fdiag, assign, ptype, nvar, temp1, temp2, pindex[sparse])
            df <- dftemp$df
            var <- dftemp$var
            var2 <- dftemp$var2
        ...
        if (nfrail > 0) { penalty <- -coxlist1$penalty } else { penalty <- 0 }
    ...
    if (andersen) {
        ...
        resid <- .Call(Cagmart3, nrow(y), y, exp(lp), weights, as.integer(strata), sorted[, 2], sorted[, 1], as.integer(method == 'efron'))
    } else {
        expect <- .C(Ccoxfit5c, as.integer(n), as.integer(nvar), as.integer(newstrat), as.integer(method == 'efron'), expect = double(n))$expect
        resid <- status - expect
    }
    ...
    warning(paste("Inner loop failed to coverge for iterations", paste(iterfail, collapse = ' ')))
    ...
    if (nfrail > 0) {
        if (nvar > 0) { #sparse frailties and covariates
            list(coefficients = coef, var = var, var2 = var2, loglik = c(loglik0, loglik1), iter = c(iter, iter2), linear.predictors = as.vector(lp), residuals = resid, means = means, method = method, class = c('coxph.penal', 'coxph'), frail = coxfit$fcoef, fvar = dftemp$fvar, df = df, df2 = dftemp$df2, penalty = c(penalty0, penalty), pterms = pterms, assign2 = assign, history = iterlist, coxlist1 = coxlist1, printfun = printfun)
        } else { #sparse frailties only
            list(loglik = c(loglik0, loglik1), iter = c(iter, iter2), linear.predictors = as.vector(lp), residuals = resid, means = means, method = method, class = c('coxph.penal', 'coxph'), frail = coxfit$fcoef, fvar = dftemp$fvar, df = df, df2 = dftemp$df2, penalty = c(penalty0, penalty), pterms = pterms, assign2 = assign, history = iterlist, printfun = printfun)
        }
    } else { #no sparse terms
        list(coefficients = coef, var = var, var2 = var2, loglik = c(loglik0, loglik1), iter = c(iter, iter2), linear.predictors = lp, residuals = resid, means = means, method = method, class = c('coxph.penal', 'coxph'), df = df, df2 = dftemp$df2, penalty = c(penalty0, penalty), pterms = pterms, assign2 = assign, history = iterlist, coxlist2 = coxlist2, printfun = printfun)
    }
}
