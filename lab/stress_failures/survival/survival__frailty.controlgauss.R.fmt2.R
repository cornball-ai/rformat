...
frailty.controlgauss <- function (opt, iter, old, fcoef, trH, loglik) {
    if (iter == 0) {
        ...
        if (!is.null(opt$theta)) {
            theta <- opt$theta #fixed theta case
        } else {
            if (is.null(opt$init)) { theta <- 1 } else { theta <- opt$init[1] }
        }
        list(theta = theta)
    } else {
        if (is.null(opt$trace)) { trace <- FALSE } else { trace <- opt$trace }
        ...
        if (iter == 1) {
            ...
            history <- rbind(old$history, as.vector(c(theta, resid, fsum, trH)))
            if (iter == 2) {
                if (all(history[, 2] > 0)) { theta <- history[2, 1] * 2 } else
                if (all(history[, 2] < 0)) {
                    theta <- history[2, 1] / 2
                } else {
                ...
                if (all(tempy > 0)) { newtheta <- 2 * max(tempx) } else
                if (all(tempy < 0)) { newtheta <- .5 * min(tempx) } else {
                    #find the latest point, and one on each side of 0
                    b1 <- (1:iter)[ord == iter]
                    if (b1 == 1) { b1 <- 2 } else
                    if (b1 == iter) { b1 <- iter - 1 }
