...
parsecovar1 <- function (flist, statedata) {
    ...
    pcut <- function (form) {
        if (length(form) == 3) {
            ...
            else if (form[[1]] == '~') { pcut(form[[2]]) } else { list(form) }
        } else list(form)
    }
    ...
    if (missing(statedata)) {
        assign("state", function (...) list(stateid = "state", values = c(...)), env1)
        assign("state", list(stateid = "state"))
    } else {
        for (i in statedata) {
            assign(i, eval(list(stateid = i)), env2)
            tfun <- eval(parse(text = paste0("function(...) list(stateid='", i, "', values=c(...))")))
            assign(i, tfun, env1)
        }
    }
    lterm <- lapply(lcut, function (x) {
        lapply(x, function (z) {
            ...
            list(left = eval(z[[2]], envir = env2),
                right = eval(z[[3]], envir = env2))
...
rightslash <- function (x) {
    if (!inherits(x, 'call')) { return(x) } else {
        if (x[[1]] == as.name('/')) { return(list(x[[2]], x[[3]])) } else
        if (x[[1]] == as.name('+') ||
...
parse_rightside <- function (rhs) {
    ...
    new <- lapply(parts, function (opt) {
        ...
        if (!is.list(opt)) { # no options for this line
            tform[[2]] <- opt
            list(formula = tform, ival = NULL, common = FALSE, shared = FALSE)
        } else {
...
termmatch <- function (f1, f2) {
    ...
    if (length(f1) == 0) {
        return(NULL) # a formula with only ~1
    }
...
parsecovar2 <- function (covar1, statedata, dformula, Terms, transitions,
    ...
    dmap <- array(seq_len(length(tmap)),
        dim = c(nterm + 1, nstate, nstate)) #unique values
        ...
        for (i in 1:length(covar1$rhs)) {
            ...
            for (x in lhs) {
                ...
                temp1 <- 1:nrow(statedata)
                    ...
                    if (any(is.na(k))) {
                        stop(x$left[is.na(k)], ": state not found")
                    }
            ...
            rindex2 <- termmatch(
                attr(terms(update(dformula, dummy)), "factors"), allterm)
    ...
    if (i > 0) {
        t2 <- t2[, -i, drop = FALSE] # transitions to 'censor' don't count
    }
    ...
    list(tmap = tmap2, inits = inits,
        mapid = cbind(from = indx1[trow], to = indx2[tcol]),
