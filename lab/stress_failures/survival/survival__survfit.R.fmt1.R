...
survfit.formula <- function (formula, data, weights, subset,
    ...
    indx <- match(c('formula', 'data', 'weights', 'subset', 'na.action', 'istate', 'id', 'cluster', "etype"), names(Call), nomatch = 0)
    temp <- Call[c(1, indx)]
    ...
    if (inherits(Y, "Surv2")) {
        ...
        if (anyNA(mf[-1])) { #ignore the response variable still found there
            if (missing(na.action)) {
                temp <- get(getOption("na.action"))(mf[-1])
            } else {
                temp <- na.action(mf[-1])
            }
    ...
    if (is.null(casewt)) { casewt <- rep(1.0, n) } else {
    ...
    if (length(ll) == 0) {
        X <- factor(rep(1, n)) # ~1 on the right
    } else {
        X <- strata(mf[ll])
    }
    ...
    if (!is.null(etype)) {
        if (attr(Y, "type") == "mcounting" || attr(Y, "type") == "mright")
        ...
        status <- factor(ifelse(status == 0, 0, as.numeric(etype)), labels = newlev)
    ...
    temp <- survfitTurnbull(X, newY, casewt, cluster = cluster, robust = robust, time0 = time0, ...)
    else if (attr(Y, 'type') == "right" || attr(Y, 'type') == "counting")
    temp <- survfitKM(X, newY, casewt, stype = stype, ctype = ctype, id = id, cluster = cluster, robust = robust, entry = entry, time0 = time0, ...)
    else if (attr(Y, 'type') == "mright" || attr(Y, "type") == "mcounting")
    temp <- survfitAJ(X, newY, weights = casewt, stype = stype, ctype = ctype, id = id, cluster = cluster, robust = robust, istate = istate, entry = entry, time0 = time0, ...)
    else {
    ...
    if (is.null(temp$states)) {
        ...
        class(temp) <- c("survfitms", "survfit")
    }
...
dim.survfit <- function (x) {
    ...
    if (is.null(x$states)) {
        d3 <- d3name <- NULL
        if (is.matrix(x$surv)) { d2 <- ncol(x$surv) } else {
    } else {
        d3 <- length(x$states)
        dp <- dim(x$pstate)
...
"[.survfit" <- function (x, ..., drop = TRUE) {
    ...
    if (ndots == 1 && length(dd) == 2) {
        ...
        if (dd[1] == 1) { j <- i; i <- 1 } else if (dd[2] == 1) j <- 1
        else {
            ...
            temp <- split(seq(along.with = x$time), rep(1:length(x$strata), x$strata))
            ...
            k <- cbind(indx1, indx2)
            for (j in c("surv", "std.err", "upper", "lower", "cumhaz", "std.chaz", "influence.surv", "influence.chaz"))
    ...
    if (is.null(x$strata)) {
        ...
        if (any(is.na(indx)))
        stop(paste("strata", paste(i[is.na(indx)], collapse = ' '), 'not matched'))
    ...
    if (!is.matrix(x[["surv"]])) { # no j dimension
        for (k in c("time", "n.risk", "n.event", "n.censor", "n.enter", "surv", "std.err", "cumhaz", "std.chaz", "upper", "lower", "influence.surv", "influence.chaz"))
        ...
        for (k in c("time", "n.risk", "n.event", "n.censor", "n.enter"))
        if (!is.null(x[[k]])) newx[[k]] <- (x[[k]])[irow]
        for (k in c("surv", "std.err", "cumhaz", "std.chaz", "upper", "lower", "influence.surv", "influence.chaz"))
...
survfit_confint <- function (p, se, logse = TRUE, conf.type, conf.int = .95,
    ...
    if (conf.type == 'plain') {
        ...
        list(lower = (sin(pmax(0, asin(sqrt(xx)) - se2 * scale))) ^ 2, upper = (sin(pmin(pi / 2, asin(sqrt(xx)) + se2))) ^ 2)
